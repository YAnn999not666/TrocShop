...
function ShoplightVideoCard({ item, isActive, isMuted, onToggleMute, onContact, onViewProfile }: VideoCardProps) {
  ...
  const handleSeek = (clientX: number) => {
    if (!progressBarRef.current || !videoRef.current || !duration || duration <= 0) return;
  ...
  return (
    <div className="w-full h-full shrink-0 snap-start relative bg-zinc-950 flex items-center justify-center overflow-hidden md:py-6 md:px-4">
      ...
      <div className="w-full h-full md:aspect-[9/16] md:h-[calc(100vh-5rem)] md:max-w-[450px] md:rounded-[2.5rem] md:border md:border-zinc-800 md:shadow-[0_0_50px_rgba(0,0,0,0.85)] relative flex items-center justify-center overflow-hidden bg-black">
        ...
        {/* Interactive Video Progress Bar (glisser à gauche/droite avec le doigt pour avancer) */}
        {duration && duration > 0 && (
          <div
            ref={progressBarRef}
            ...
            className="absolute bottom-0 left-0 right-0 h-6 z-30 flex items-end cursor-pointer touch-none select-none group"
            title="Glisser vers la gauche ou la droite pour avancer ou reculer"
          >
            <div className="w-full h-2 group-hover:h-3 transition-all relative bg-white/20">
              <div
                className="h-full bg-gradient-to-r from-amber-500 to-orange-500"
                style={{ width: `${Math.min(100, Math.max(0, (currentTime / duration) * 100))}%` }}
              />
              <div
                className={`absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-4 h-4 bg-white rounded-full shadow-lg border-2 border-orange-500 transition-transform ${
                  isScrubbing ? 'scale-125 bg-orange-100' : 'scale-100 group-hover:scale-125'
                }`}

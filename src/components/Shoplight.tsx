...
export function Shoplight({ onContactSeller, onBackToHome, onViewProfile }: ShoplightProps) {
  ...
  // Scroll to active index on initial load
  useEffect(() => {
    if (containerRef.current && items.length > 0 && activeIndex > 0 && !initialScrollDone.current) {
      initialScrollDone.current = true;
      const container = containerRef.current;
      const scrollToActive = () => {
        const clientHeight = container.clientHeight;
        if (clientHeight > 0) {
          container.scrollTop = activeIndex * clientHeight;
        } else {

...
export const SellerListingsManager: React.FC<SellerListingsManagerProps> = ({
  ...
  // Helper for badge rendering (optimized for smooth scrolling without backdrop-blur filters or pulse loops)
  const renderStatusBadge = (p: Product) => {
    ...
    if (isSold) {
      return (
        <span className="px-2.5 py-1 bg-zinc-900 text-white text-[10px] font-black uppercase tracking-wider rounded-lg border border-white/20 shadow-xs flex items-center gap-1">
    ...
    if (isReserved) {
      return (
        <span className="px-2.5 py-1 bg-amber-500 text-zinc-950 text-[10px] font-black uppercase tracking-wider rounded-lg border border-amber-400/40 shadow-xs flex items-center gap-1">
          <Clock size={12} />
          Réservé
        </span>
      );
    ...
    if (isOutOfStock) {
      return (
        <span className="px-2.5 py-1 bg-rose-600 text-white text-[10px] font-black uppercase tracking-wider rounded-lg border border-white/20 shadow-xs flex items-center gap-1">
          <PackageX size={12} />
          Rupture
        </span>
      );
    ...
    if (isPaused) {
      return (
        <span className="px-2.5 py-1 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-wider rounded-lg border border-white/20 shadow-xs flex items-center gap-1">
          <PauseCircle size={12} />
          En pause
        </span>
      );
    ...
    if (isExpired) {
      return (
        <span className="px-2.5 py-1 bg-zinc-700 text-white text-[10px] font-black uppercase tracking-wider rounded-lg border border-white/10 shadow-xs">
          Expiré
        </span>
      );
    }
    if (isUrgent) {
      return (
        <span className="px-2.5 py-1 bg-red-600 text-white text-[10px] font-black uppercase tracking-wider rounded-lg border border-white/20 shadow-xs flex items-center gap-1">
    ...
    if (isPriceLowered) {
      return (
        <span className="px-2.5 py-1 bg-emerald-600 text-white text-[10px] font-black uppercase tracking-wider rounded-lg border border-white/20 shadow-xs flex items-center gap-1">
    ...
    return (
      <span className="px-2.5 py-1 bg-emerald-500 text-white text-[10px] font-black uppercase tracking-wider rounded-lg border border-white/20 shadow-xs flex items-center gap-1">
        <CheckCircle size={12} />
        Actif
      </span>
    );
    ...
    return (
      ...
      >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          <div className="bg-white p-4 sm:p-5 rounded-[1.8rem] border border-orange-500/20 shadow-xs hover:border-orange-500/40 transition-colors space-y-2 group">
            <div className="w-10 h-10 rounded-2xl bg-orange-500/10 text-orange-600 flex items-center justify-center border border-orange-500/20 shadow-xs transition-transform">
              <BarChart2 size={20} />
            </div>
            <div>
          ...
          <div className="bg-white p-4 sm:p-5 rounded-[1.8rem] border border-red-500/20 shadow-xs hover:border-red-500/40 transition-colors space-y-2 group">
            <div className="w-10 h-10 rounded-2xl bg-red-500/10 text-red-500 flex items-center justify-center border border-red-500/20 shadow-xs transition-transform">
          ...
          <div className="bg-white p-4 sm:p-5 rounded-[1.8rem] border border-blue-500/20 shadow-xs hover:border-blue-500/40 transition-colors space-y-2 group">
            <div className="w-10 h-10 rounded-2xl bg-blue-500/10 text-blue-600 flex items-center justify-center border border-blue-500/20 shadow-xs transition-transform">
          ...
          <div className="bg-white p-4 sm:p-5 rounded-[1.8rem] border border-emerald-500/20 shadow-xs hover:border-emerald-500/40 transition-colors space-y-2 group">
            <div className="w-10 h-10 rounded-2xl bg-emerald-500/10 text-emerald-600 flex items-center justify-center border border-emerald-500/20 shadow-xs transition-transform">
              <Clock size={20} />
            </div>
            <div>
  ...
  return (
    ...
    >
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-[2.2rem] p-6 text-white shadow-lg relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-orange-600/20 to-transparent rounded-full pointer-events-none" />
          ...
          {filteredProducts.map((product, index) => {
            ...
            return (
              <div
                ...
                className={cn(
                  "bg-white rounded-[2rem] border border-zinc-150 shadow-xs hover:shadow-md transition-shadow duration-150 flex flex-col relative group cursor-pointer overflow-hidden",
              ...
              >
                ...
                <div className="p-3.5 space-y-2 flex-1 flex flex-col justify-between">
                  ...
                  {/* Reservation Action Bar for Seller - Only Discuter button */}
                  {(product.status === 'reserved' || product.status === 'pending') && (
                    <div className="pt-2 border-t border-zinc-100">
                      {onOpenConversation && (
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            const conv = conversations.find(
                              (c: any) =>
                                ...
                                (product.reserved_by &&
                                  (c.participantIds?.includes(product.reserved_by) || c.buyerId === product.reserved_by))
                            );
                            if (conv) {
                          ...
                          className="w-full py-2 px-3 bg-zinc-900 hover:bg-black text-white rounded-xl text-[11px] font-black flex items-center justify-center gap-1.5 transition-colors cursor-pointer shadow-xs"

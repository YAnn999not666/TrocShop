...
export function ServicesPage({ onBackToHome, user, onContactSeller, onDetailToggle }: ServicesPageProps) {
  ...
  const handleShare = (item: ServiceListing) => {
    ...
    const shareText = `Découvre cette ${typeLabel} en location sur TrocShop : "${item.title}" situé à ${item.neighborhood} pour seulement ${item.price.toLocaleString()} FCFA par ${periodLabel}.`;
                ...
                {filteredCars.map((car) => (
                  ...
                  >
                    ...
                    <div className="p-5 space-y-2">
                      ...
                      <p className="text-xs text-zinc-500 line-clamp-2 font-medium">{car.description}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
                ...
                {filteredHouses.map((house) => (
                  ...
                  >
                    ...
                    <div className="p-5 space-y-2">
                      ...
                      <p className="text-xs text-zinc-500 line-clamp-2 font-medium">{house.description}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 bg-zinc-50 p-4 rounded-2xl border border-zinc-100">
                <div className="w-10 h-10 rounded-full bg-zinc-200 text-zinc-500 flex items-center justify-center shrink-0">
                  <User size={18} />
                </div>
                <div>
                  <h4 className="text-[10px] font-black uppercase text-zinc-400 tracking-wider">Publié par</h4>
                  <p className="text-xs font-bold text-zinc-800 mt-0.5">{selectedHouse.sellerName}</p>
                </div>
              </div>
      ...
      <AnimatePresence>
        {isLightboxOpen && selectedHouse && selectedHouse.images && selectedHouse.images.length > 0 && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex flex-col justify-between"

...
export const ProductCardSkeleton = () => (
  <div className="bg-white/90 backdrop-blur-md rounded-[2.2rem] overflow-hidden border border-zinc-150/80 shadow-sm transition-all">
...
export const ProductCard = ({ 
  ...
  return (
    <motion.div 
      ...
      className={cn(
        "group bg-white/90 backdrop-blur-md rounded-[2.2rem] overflow-hidden border transition-all duration-500 relative flex flex-col justify-between",
        isDisabled 
          ? "cursor-default border-zinc-200/80 opacity-90 shadow-sm" 
          : "cursor-pointer border-zinc-200/60 hover:border-orange-500/30 hover:shadow-[0_20px_40px_rgba(234,88,12,0.12)] hover:-translate-y-1"
      )}
    >
      {/* Top Left Badge */}
      <div className="absolute top-3.5 left-3.5 z-10">
        <span className={cn(
          "px-3 py-1.5 rounded-xl backdrop-blur-xl text-[9px] font-black uppercase tracking-wider text-white border border-white/30 shadow-lg select-none flex items-center gap-1.5",
          product.listingType === 'sale' 
            ? "bg-emerald-600/90 shadow-emerald-600/20" :
          product.listingType === 'mixed' 
            ? "bg-gradient-to-r from-orange-600 to-amber-500 shadow-orange-600/30" 
      ...
      <div className="absolute top-3.5 right-3.5 z-10 flex flex-col gap-2">
        <motion.button 
          ...
          disabled={isDisabled}
          className={cn(
            "px-3 py-2 rounded-full backdrop-blur-xl flex items-center gap-1.5 border shadow-md transition-all",
            isDisabled 
              ...
              : favorite 
                ...
                : "bg-white/85 border-white/60 hover:bg-white text-zinc-500 hover:text-red-500"
          )}
        >
          <Heart className={cn("w-4 h-4 transition-transform duration-300", favorite ? "fill-red-500 text-red-500 scale-110" : "text-zinc-500")} />
      ...
      {/* Image Container */}
      <div className="aspect-[4/5] relative overflow-hidden bg-zinc-100/80 group">
        <img 
          src={imageUrl} 
          alt={product.title}
          loading="lazy"
        ...
        <div className="absolute bottom-3 left-3 right-3 flex flex-wrap gap-1.5 z-10 items-center justify-start pointer-events-none">
          <div className="bg-zinc-950/80 backdrop-blur-xl border border-white/20 px-3 py-1.5 rounded-xl text-[8px] font-black uppercase tracking-widest text-white shadow-lg w-fit">
            {product.condition}
          </div>

          {(product as any).isUrgent && (
            <div className="bg-red-600/90 backdrop-blur-xl border border-red-400/40 px-2.5 py-1.5 rounded-xl text-[8px] font-black uppercase tracking-widest text-white shadow-lg shadow-red-600/30 flex items-center gap-1 w-fit animate-pulse">
          ...
          {(product as any).isPriceLowered && (
            <div className="bg-emerald-600/90 backdrop-blur-xl border border-emerald-400/40 px-2.5 py-1.5 rounded-xl text-[8px] font-black uppercase tracking-widest text-white shadow-lg flex items-center gap-1 w-fit">
          ...
          {(product.status === 'sold' || product.is_available === false) ? (
            <div className="bg-red-600/95 backdrop-blur-xl border border-white/20 px-3 py-1.5 rounded-xl text-[8px] font-black uppercase tracking-widest text-white shadow-lg flex items-center gap-1.5 w-fit">
              <ShoppingBag size={10} />
              Vendu
            </div>
          ) : (
            (product.status === 'pending' || product.transactionInProgress || (product as any)['transactionInProgress'] === true) && (
              <div className="bg-amber-500/95 text-zinc-950 backdrop-blur-xl border border-white/30 px-3 py-1.5 rounded-xl text-[8px] font-black uppercase tracking-widest shadow-lg flex items-center gap-1.5 w-fit animate-pulse">
                <Clock size={10} />
                Réservé
              </div>
            )
      ...
      {/* Card Info Footer */}
      <div className="p-4 flex items-center justify-between gap-2 bg-white/95">

...
import { SellerListingsManager } from "./components/SellerListingsManager";
import UserNetworkManager from "./components/UserNetworkManager";
import { SpotlightPublisher } from "./components/SpotlightPublisher";
import { LuxuryCarousel } from "./components/LuxuryCarousel";
import { logProductView } from "./lib/productEvents";
// import { SellServiceForm } from "./components/SellServiceForm";
import {
  auth,
  db,
...
import { Shoplight } from "./components/Shoplight";
...
const PublicSpotlightPlayer = ({
  ...
  sellerId?: string;
  sellerName?: string;
  onContactSeller?: (sellerId: string, sellerNom: string, item: any) => void;
}) => {
  ...
  const handleShare = async (e: React.MouseEvent) => {
    ...
    const nameToUse = sellerName || item.vendeur_nom || "un membre";
    const text = `Regarde la vidéo de ${nameToUse} sur TrocShop : "${item.description?.substring(0, 60) || ""}..."`;
    
    try {
      if (navigator.share) {
        await navigator.share({
          title: "TrocShop Shoplight",
          text: text,
          url: shareUrl
        });
      } else {
  ...
  return (
    <div className="bg-zinc-50 border border-zinc-150 rounded-[2.5rem] p-6 space-y-4 max-w-sm mx-auto shadow-sm">
      <div className="flex items-center justify-between">
        ...
        <div className="bg-orange-500/10 text-orange-600 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest">
          En Ligne
        </div>
      </div>
      ...
      <div
        onClick={togglePlay}
        className="aspect-[9/16] w-full rounded-[2rem] bg-zinc-950 overflow-hidden relative shadow-lg group cursor-pointer border border-zinc-200"
      >
        <video
          ref={videoRef}
          src={item.video_url}
        ...
        <div className="absolute inset-0 p-4 flex flex-col justify-between">
          <div className="flex justify-end">
            <button
              type="button"
              onClick={toggleMute}
              className="p-2.5 rounded-full bg-black/45 backdrop-blur-md text-white border border-white/10 active:scale-90 transition-all hover:bg-black/60"
            >
              {isMuted ? (
            ...
            <AnimatePresence>
              {!isPlaying && (
                <motion.div
                  ...
                  className="p-4 rounded-full bg-orange-600 border border-orange-500 text-white shadow-xl pointer-events-auto"
                >
          ...
          {/* Right Column overlay for Actions */}
          <div className="absolute right-4 bottom-4 flex flex-col items-center gap-3 z-10">
            {/* Contact button */}
            {sellerId && onContactSeller && (
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation();
                  onContactSeller(sellerId, sellerName || "Vendeur", item);
                }}
                className="p-3 rounded-full bg-orange-600 hover:bg-orange-700 text-white shadow-lg active:scale-90 transition-all hover:scale-105 border border-orange-500/50 flex items-center justify-center"
                title="Discuter"
              >
            ...
            {sellerProfile?.phoneVisibility === "public" && sellerProfile?.phoneNumber && (
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  handlePhoneCall(sellerProfile.phoneNumber);
                }}
                className="p-3 rounded-full bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg active:scale-90 transition-all hover:scale-105 border border-emerald-500/50 flex items-center justify-center animate-bounce"
                title={`Appeler le vendeur : ${sellerProfile.phoneNumber}`}
              >
                <Phone size={16} className="fill-white/10 text-white" />
              </button>
            )}
            ...
            <button
              type="button"
              onClick={handleShare}
              className="p-3 rounded-full bg-black/45 backdrop-blur-md hover:bg-black/60 text-white border border-white/10 shadow-lg active:scale-90 transition-all hover:scale-105 flex items-center justify-center"
              title="Partager"
            >
              <Share size={16} />
            </button>
          ...
          {/* Description overlay */}
          <div className="z-10 mt-auto text-white pointer-events-none space-y-1 max-w-[70%]">
            <p className="text-[11px] font-medium leading-relaxed drop-shadow-md line-clamp-3 text-zinc-100">
              {item.description}
            </p>
          </div>
        </div>
...
const ConversationRecipientAvatar = ({
  participantNames,
  participantIds,
  currentUser,
}: {
  ...
  const defaultLetter = (participantNames?.find(
  ...
  if (profile?.photoURL) {
    return (
      <img
        ...
        className="w-14 h-14 rounded-[1.2rem] object-cover border-2 border-white shadow-sm shrink-0"
        alt="Avatar"
      />
    );
  }
...
const ConversationRecipientName = ({
  ...
  const otherId = participantIds?.find((id) => id !== currentUser?.uid);
  const defaultName =
    participantNames?.find((n) => n !== currentUser?.displayName) ||
    `Utilisateur ${otherId?.slice(0, 4) || ""}`;
  const [profile, setProfile] = useState<any>(null);
  ...
  const isCertified =
    (profile?.is_certified as any) === true ||
    (profile?.is_certified as any) === "vrai" ||
    (profile?.isCertified as any) === true ||
    (profile?.isCertified as any) === "vrai" ||
    (profile?.["is certified"] as any) === true ||
    (profile?.["is certified"] as any) === "vrai" ||
    profile?.isPro === true ||
    profile?.is_pro === true ||
    (profile?.badges as string[])?.some((b) =>
      b?.toLowerCase().includes("certif"),
    );
  ...
  return (
    <div className="flex items-center gap-1.5 min-w-0">
      ...
      {isCertified && (
        <BadgeCheck
          size={14}
          className="text-orange-600 fill-orange-600 text-white shrink-0"
        />
      )}
      {profile?.isStudent && (
        <span
          className="shrink-0 p-1 bg-blue-50 text-blue-700 border border-blue-100 rounded-full flex items-center justify-center"
          title={`Étudiant à ${profile.studentSchool || ""}`}
        >
    ...
    (Array.isArray(data.badges) &&
      data.badges.some(
        (b: any) =>
          typeof b === "string" &&
          (b.toLowerCase().includes("certif") ||
            b.toLowerCase().includes("valide") ||
            b.toLowerCase().includes("vérifié") ||
            b.toLowerCase().includes("verifie")),
      ))
  );
  const isStudent = !!(
    data.isStudent === true ||
    ...
    (Array.isArray(data.badges) &&
      data.badges.some(
        (b: any) =>
          typeof b === "string" &&
          (b.toLowerCase().includes("étudiant") ||
            b.toLowerCase().includes("etudiant")),
      ))
  );
...
export default function App() {
  ...
  useEffect(() => {
    // Check if running standalone or if banner was dismissed
  ...
  const handleInstallClick = async () => {
    if (!deferredPrompt) {
      alert(
        "Pour installer TrocShop, ouvrez le menu de votre navigateur (icône de partage ou les trois points) et sélectionnez 'Ajouter à l'écran d'accueil' ou 'Installer l'application'.",
      );
      setShowInstallBanner(false);
  ...
  const [updateDownloadUrl, setUpdateDownloadUrl] = useState(
    "https://troc-shop-kgfn.vercel.app/",
  );
    ...
    const unsubscribe = onSnapshot(
      ...
      (err) => {
        if (!navigator.onLine) {
          ...
          console.warn("Failed to listen to app version:", err);
        }
      },
    );
  ...
  const [explorerPageLastDocs, setExplorerPageLastDocs] = useState<
    Record<number, any>
  >({});
  ...
  const explorerProducts = useMemo(() => {
    ...
    const keys = Object.keys(explorerPages)
      .map(Number)
      .sort((a, b) => a - b);
    keys.forEach((k) => {
      if (explorerPages[k]) {
  ...
  const updateLocalProductState = (
    ...
    const ids = Array.isArray(productIdOrIds)
      ? productIdOrIds
      : [productIdOrIds];
    ...
    setExplorerPages((prev) => {
      ...
      Object.keys(next).forEach((pageKey) => {
        ...
        if (next[pageIndex]) {
          next[pageIndex] = next[pageIndex].map((p) =>
            ids.includes(p.id) ? { ...p, ...updates } : p,
          );
        }
      });
  ...
  const [filterStudentSchool, setFilterStudentSchool] =
    useState<string>("Tous");
  const [filterCertifiedOnly, setFilterCertifiedOnly] =
    useState<boolean>(false);

  // Student space view search filters
  const [studentSearchQuery, setStudentSearchQuery] = useState("");
  const [studentFilterSchool, setStudentFilterSchool] =
    useState<string>("Tous");
  const [studentFilterCondition, setStudentFilterCondition] =
    useState<string>("Tous");
  const [studentFilterPriceMax, setStudentFilterPriceMax] = useState<
    number | null
  >(null);
  const [studentFilterListingType, setStudentFilterListingType] =
    useState<string>("Tous");
  const [studentSortBy, setStudentSortBy] = useState<
    "newest" | "price-asc" | "price-desc" | "oldest"
  >("newest");
  const [isStudentFiltersOpen, setIsStudentFiltersOpen] = useState(true);

  // Troc space view search filters
  const [trocSearchQuery, setTrocSearchQuery] = useState("");
  const [trocFilterCondition, setTrocFilterCondition] =
    useState<string>("Tous");
  const [trocFilterPriceMax, setTrocFilterPriceMax] = useState<number | null>(
    null,
  );
  const [trocFilterListingType, setTrocFilterListingType] =
    useState<string>("Tous");
  const [trocFilterNeighborhood, setTrocFilterNeighborhood] =
    useState<string>("Tous");
  const [trocSortBy, setTrocSortBy] = useState<
    "newest" | "price-asc" | "price-desc" | "oldest"
  >("newest");
  const [isTrocFiltersOpen, setIsTrocFiltersOpen] = useState(true);
  ...
  const [categoryFilterCondition, setCategoryFilterCondition] =
    useState<string>("Tous");
  const [categoryFilterPriceMax, setCategoryFilterPriceMax] = useState<
    number | null
  >(null);
  const [categoryFilterListingType, setCategoryFilterListingType] =
    useState<string>("Tous");
  ...
  const [certifiedSellers, setCertifiedSellers] = useState<
    Record<string, boolean>
  >({});
  const [isFiltersOpen, setIsFiltersOpen] = useState(false);
  const [peekedNotificationId, setPeekedNotificationId] = useState<
    string | null
  >(null);

  const [isMessagesEditMode, setIsMessagesEditMode] = useState(false);
  const [selectedMessages, setSelectedMessages] = useState<string[]>([]);
  const [peekedConversationId, setPeekedConversationId] = useState<
    string | null
  >(null);
  ...
  const [justGeneratedCodeExIds, setJustGeneratedCodeExIds] = useState<
    string[]
  >([]);
  const [mytrocSubTab, setMytrocSubTab] = useState<"ongoing" | "past">(
    "ongoing",
  );
  ...
  useEffect(() => {
    ...
    if (
      ...
      (window as any).__firestore_quota_exceeded
    ) {
      setFirestoreQuotaExceeded(true);
    }
    return () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener("view-seller-profile", handleViewProfile);
    return () => {
      window.removeEventListener("view-seller-profile", handleViewProfile);
    };
  }, [user]);
  ...
  const [initialVideoId, setInitialVideoId] = useState<string | null>(() => {
    if (typeof window !== "undefined") {
  ...
  const [initialProfileId, setInitialProfileId] = useState<string | null>(() => {
    if (typeof window !== "undefined") {
  ...
  const [initialServiceId, setInitialServiceId] = useState<string | null>(() => {
    if (typeof window !== "undefined") {
  ...
  // Clean up URL search params once deep link params are consumed so reloads/reopening doesn't re-trigger them
  useEffect(() => {
  ...
  // Auto-select profile on deep link startup
  useEffect(() => {
    if (initialProfileId) {
      setSelectedSellerId(initialProfileId);
    }
  }, [initialProfileId]);

  // Auto-select product on deep link startup
  useEffect(() => {
  ...
  useEffect(() => {
    if (selectedProduct?.id) {
      logProductView(selectedProduct.id, user?.uid);
    }
  }, [selectedProduct?.id, user?.uid]);
  const [editingProduct, setEditingProduct] = useState<Product | null>(null);
  const [editingService, setEditingService] = useState<any | null>(null);
  const [loading, setLoading] = useState(true);

  const [selectedCategory, setSelectedCategory] = useState("Tous");
  const [allUsersProfiles, setAllUsersProfiles] = useState<
  ...
  const [profileView, setProfileView] = useState<
    "main" | "listings" | "favorites" | "settings" | "security" | "network"
  >("main");
  const [listingsTab, setListingsTab] = useState<"products" | "services">(
    "products",
  );
      ...
      try {
        ...
        if (result?.user) {
          ...
          if (storedMode === "signup" && snap.exists()) {
            alert(
              "Vous avez déjà un compte Google associé. Connectez-vous plutôt.",
            );
            setAuthMode("login");
            return;
          }
          if (storedMode === "login" && !snap.exists()) {
            alert(
              "Aucun compte Google trouvé. Veuillez vous inscrire avec ce compte d'abord.",
            );
            setAuthMode("signup");
            return;
          }
        ...
        console.error(
          "Redirect auth result processing error:",
  ...
  const fetchExplorerProducts = async () => {
    try {
      setExplorerLoading(true);
      let queryRes = await supabase
        .from('products')
        .select('*')
        .order('is_pinned', { ascending: false }) // 1. Les épinglés d'abord
        .order('created_at', { ascending: false }); // 2. Les plus récents ensuite

      if (queryRes.error) {
        console.warn("Falling back to JSONB ordering for is_pinned in explorer:", queryRes.error);
        queryRes = await supabase
          .from('products')
          .select('*')
          .order('data->is_pinned', { ascending: false })
      ...
      if (queryRes.data) {
        const mapped = queryRes.data.map((row: any) => ({
          id: String(row.id),
          ...row.data,
          ...
          status: row.status,
          isPinned: row.data?.is_pinned ?? row.is_pinned ?? false,
          is_pinned: row.data?.is_pinned ?? row.is_pinned ?? false,
        })) as Product[];

        globalDataCache["explorerPage_0"] = {
          pList: mapped,
          lastDoc: null,
          hasMore: false,
        };
        ...
        try {
          localStorage.setItem("cached_explorer_page_0", JSON.stringify(mapped));
        } catch (e) {}

        setProducts(mapped);
        setExplorerPages({ 0: mapped });
        setExplorerHasMore(false);
        setExplorerLoading(false);
      }
    } catch (error) {
      console.error("Erreur fetchExplorerProducts:", error);
      setExplorerLoading(false);

      // Handle gracefully: fall back to cached products if available in localStorage
      try {
        const cached = localStorage.getItem("cached_explorer_page_0");
        if (cached) {
          const parsed = JSON.parse(cached);
          if (Array.isArray(parsed) && parsed.length > 0) {
            setProducts(parsed);
            setExplorerPages({ 0: parsed });
            setExplorerHasMore(false);
          }
        }
      } catch (e) {
        console.warn("Failed to load cached explorer products:", e);
      }
    }
  };
  ...
  const fetchHomeProducts = async () => {
    try {
      let queryRes = await supabase
        ...
        .order('is_pinned', { ascending: false }) // 1. Les épinglés d'abord
        .order('created_at', { ascending: false }) // 2. Les plus récents ensuite
        .limit(10); // Limite à 10 produits pour l'accueil

      if (queryRes.error) {
        console.warn("Falling back to JSONB ordering for is_pinned in home:", queryRes.error);
        queryRes = await supabase
          .from('products')
          .select('*')
          .order('data->is_pinned', { ascending: false })
      ...
      if (queryRes.data) {
        ...
        globalDataCache["homeProducts"] = mapped;
        setHomeProducts(mapped);
        setIsInitialLoading(false);
        // Sauvegarde optionnelle dans le cache local pour éviter les lectures inutiles au rechargement
        localStorage.setItem('cached_home_products', JSON.stringify(mapped));
      }
    } catch (error) {
  ...
  useEffect(() => {
    // Load explorer products and home products on mount
    fetchExplorerProducts();
    fetchHomeProducts();
  }, []);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (err) => {
        console.warn("Error subscribing to services in App:", err);
      },
    );
    return () => unsub();
  }, []);
  ...
  const handleDeleteService = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    safeConfirm(
      "Voulez-vous vraiment supprimer cette annonce de service ?",
      async () => {
        setAllServices((prev) => prev.filter((s) => s.id !== id));
        try {
          const docRef = doc(db, "services", id);
          await deleteDoc(docRef);
          safeAlert("Annonce de service supprimée.");
        } catch (err) {
      ...
      try {
        const chunkPromises = chunks.map(async (chunk) => {
          ...
          snap.forEach((docSnap) => {
            ...
            globalUserProfileCache[docSnap.id] = {
              uid: docSnap.id,
              ...profileData,
            };
            chunkMap[docSnap.id] = {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (scrollHeight - scrollTop - clientHeight < 300) {
        ...
        const lastDoc =
          lastPageIndex >= 0 ? explorerPageLastDocs[lastPageIndex] : null;
  ...
  const [publicProfileTab, setPublicProfileTab] = useState<
    "boutique" | "services" | "shoplight"
  >("boutique");
  const [publicSearchQuery, setPublicSearchQuery] = useState("");
  const [publicCategoryFilter, setPublicCategoryFilter] = useState("");
  const [publicMinPrice, setPublicMinPrice] = useState("");
  const [publicMaxPrice, setPublicMaxPrice] = useState("");
  const [publicSortBy, setPublicSortBy] = useState("recent");
  const [publicShowAdvancedFilters, setPublicShowAdvancedFilters] =
    useState(false);
  const [publicSpotlights, setPublicSpotlights] = useState<any[]>([]);

  // Follow state for selected seller profile
  const [sellerFollowersCount, setSellerFollowersCount] = useState<number>(0);
  const [sellerFollowingCount, setSellerFollowingCount] = useState<number>(0);
  const [isFollowingSeller, setIsFollowingSeller] = useState<boolean>(false);
  const [isFollowLoading, setIsFollowLoading] = useState<boolean>(false);

  // Follow state for logged-in user
  const [myFollowersCount, setMyFollowersCount] = useState<number>(0);
  const [myFollowingCount, setMyFollowingCount] = useState<number>(0);

  // Fetch follow statistics & check status for selected seller
  useEffect(() => {
    if (!selectedSellerId) {
      setSellerFollowersCount(0);
      setSellerFollowingCount(0);
    ...
    async function loadSellerFollowStats() {
      try {
        // 1. Count followers for selected seller
        const { count: followersC, error: err1 } = await supabase
          .from("follows")
          .select("*", { count: "exact", head: true })
          .eq("followed_id", selectedSellerId);

        // 2. Count followings for selected seller
        const { count: followingC, error: err2 } = await supabase
          .from("follows")
          .select("*", { count: "exact", head: true })
          .eq("follower_id", selectedSellerId);

        if (isMounted) {
          if (!err1 && followersC !== null) setSellerFollowersCount(followersC || 0);
          if (!err2 && followingC !== null) setSellerFollowingCount(followingC || 0);
        }

        // 3. Check if current logged-in user follows this seller
        if (user?.uid && user.uid !== selectedSellerId) {
          ...
          if (isMounted) setIsFollowingSeller(false);
        }
      } catch (e) {
        console.warn("Error fetching seller follow stats from Supabase:", e);
      }
    }

    loadSellerFollowStats();

    return () => {
      isMounted = false;
    };
  }, [selectedSellerId, user?.uid]);

  // Fetch follow statistics for logged-in user
  useEffect(() => {
    if (!user?.uid) {
    ...
    async function loadMyFollowStats() {
      try {
        ...
        if (isMounted) {
          if (followersC !== null) setMyFollowersCount(followersC || 0);
          if (followingC !== null) setMyFollowingCount(followingC || 0);
        }
      } catch (e) {
        console.warn("Error fetching my follow stats from Supabase:", e);
      }
    }
  ...
  // Handler to toggle follow/unfollow
  const handleToggleFollowSeller = async () => {
    if (!user?.uid) {
      triggerAuthRequired("Connectez-vous pour suivre ce vendeur et recevoir des notifications lors de ses nouvelles publications.");
      return;
    }

    if (!selectedSellerId || selectedSellerId === user.uid) return;
    ...
    if (isFollowingSeller) {
      // Unfollow
      setIsFollowingSeller(false);
      setSellerFollowersCount((prev) => Math.max(0, prev - 1));
      ...
      if (error) {
        ...
        setSellerFollowersCount((prev) => prev + 1);
      } else {
        toast.success(`Vous ne suivez plus ${publicProfile?.displayName || 'ce vendeur'}.`);
      ...
      if (error) {
        ...
        toast.success(`Vous suivez désormais ${publicProfile?.displayName || 'ce vendeur'} !`);
  ...
  useEffect(() => {
    if (!selectedSellerId) {
      setPublicReviews([]);
      setPublicProfileTab("boutique");
      setPublicSearchQuery("");
      setPublicCategoryFilter("");
      setPublicMinPrice("");
      setPublicMaxPrice("");
      setPublicSortBy("recent");
      setPublicShowAdvancedFilters(false);
      setPublicSpotlights([]);
      return;
    }
    setPublicProfileTab("boutique");
  ...
  useEffect(() => {
    ...
    async function fetchUserSpotlights() {
      try {
        const { data, error } = await supabase
          .from("spotlight")
          .select("*")
          .eq("vendeur_id", selectedSellerId)
          .or("expires_at.is.null,expires_at.gt." + new Date().toISOString())
          .order("created_at", { ascending: false });
        if (error) throw error;
        setPublicSpotlights(data || []);
      } catch (err) {
        console.warn(
          "Error fetching public user spotlights from supabase:",
          err,
        );
        setPublicSpotlights([]);
      }
  ...
  useEffect(() => {
    const loadSellerProfile = async () => {
      ...
      if (
        globalDataCache[cacheKeyProfile] !== undefined &&
        globalDataCache[cacheKeyProducts] !== undefined
      ) {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          const pList = productsSnap.docs.map(
  ...
  const handleEmailLogin = async (
    ...
    try {
      ...
      if (
        ...
        alert("Nom, prénom ou mot de passe incorrect. Si vous n'avez pas de compte, veuillez vous inscrire.");
      } else {
        alert("Erreur de connexion : " + (e.message || String(e)));
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleEmailSignup = async (data: any) => {
    ...
    try {
      ...
      const newProfile: UserProfile = {
        ...
        studentSchool: data.isStudent
          ? data.studentSchool || CAMPUS_SCHOOLS[0]
          : "",
      };
      ...
      setActiveTab("home");
      // setShowSecurityRulesModal(true); // Supprimé comme demandé pour ne plus afficher ce message à l'inscription
    } catch (e: any) {
      ...
      if (
        ...
        setAuthFailedHelp(
          "Problème de connexion Firebase détecté (auth/network-request-failed) pendant l'inscription.\n\nCela se produit lorsque le navigateur bloque l'authentification et l'écriture dans l'iframe.\n\n👉 ACTION REQUISE : Ouvrez l'application dans un nouvel onglet indépendant en utilisant la barre ci-dessous.",
        );
      } else if (e.code === "auth/email-already-in-use") {
        alert(
          "Ce nom et prénom sont déjà utilisés. Essayez de vous connecter.",
        );
        setAuthMode("login");
      } else if (e.code === "auth/operation-not-allowed") {
        alert(
          "🚨 Action Requise: Le fournisseur 'Email/Mot de passe' doit être activé dans votre console Firebase.\n\nPour corriger :\n1. Allez sur https://console.firebase.google.com/project/" +
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (
        ...
        setAuthFailedHelp(
          "L'ouverture de l'authentification Google a échoué (auth/network-request-failed).\n\nCela se produit souvent car l'application s'exécute dans une iframe qui restreint les Cookies ou l'authentification POPUP.\n\n👉 FIABILISATION : Ouvrez l'application dans un nouvel onglet indépendant.",
        );
      } else if (e?.code === "auth/popup-blocked") {
        alert(
          "La fenêtre de connexion Google a été bloquée. Veuillez ouvrir l'application dans un nouvel onglet ou autoriser les pop-ups (en haut dans la barre d'adresse) pour vous connecter avec Google.",
        );
      } else if (
  ...
  const handleDeleteProduct = async (id: string, e: React.MouseEvent) => {
    ...
    safeConfirm("Voulez-vous vraiment supprimer cette annonce ?", async () => {
      ...
      if (globalDataCache["homeProducts"]) {
        globalDataCache["homeProducts"] = globalDataCache[
          "homeProducts"
  ...
  // Auto-select video / shoplight on deep link startup
  useEffect(() => {
    if (initialVideoId) {
      setActiveTab("shoplight");
    }
  }, [initialVideoId]);

  // Auto-select conversation on deep link startup
  useEffect(() => {
  ...
  const toggleFavorite = async (id: string, e?: React.MouseEvent) => {
    ...
    if (p) {
      const isReservedActive =
        ...
        (!p.reserved_until && !(p as any).reservedUntil
          ? true
          : new Date(p.reserved_until || (p as any).reservedUntil).getTime() > Date.now());
      if (isReservedActive) {
        return;
      }
    }
    if (!user) {
      triggerAuthRequired(
        "Pour aimer cet article et l'ajouter à vos favoris, vous devez d'abord vous connecter ou créer un compte.",
      );
      return;
    }
  ...
  useEffect(() => {
    ...
    const countPendingExchanges = exchanges.filter(
      (e) =>
            ...
            if (now - lastBackPressRef.current < 2000) {
              ...
              setTimeout(() => {
                setInAppNotification((prev) =>
                  prev?.title === "Quitter l'application 🚪" ? null : prev,
                );
              }, 2000);
            }
          }
  ...
  useEffect(() => {
        ...
        if (!isFirstExchangesLoadRef.current) {
          snap.docChanges().forEach((change) => {
            if (change.type === "modified") {
              ...
              setTimeout(() => {
                setInAppNotification((prev) =>
                  prev?.body.includes(exData.productTitle) ? null : prev,
                );
              }, 6000);
            }
          });
    ...
    const unsubSeller = onSnapshot(
      ...
      (snap) => {
        ...
        // Troc notifications suppressed as requested
      },
      (e) => {
        if (!e.message?.includes("index")) {
  ...
  useEffect(() => {
    ...
    exchanges.forEach(async (ex) => {
      ...
      const uniqueMyIds = Array.from(
        new Set(myAffectedProductIds.filter(Boolean)),
      );
      ...
      for (const prodId of uniqueMyIds) {
        ...
        const localProd =
          ...
          homeProducts.find((p) => p.id === prodId);

        if (
          !localProd ||
          localProd.status !== "sold" ||
          localProd.is_available !== false ||
          localProd.transactionInProgress
        ) {
          try {
            ...
            if (snap.exists()) {
              ...
              if (
                dbData.status !== "sold" ||
                dbData.is_available !== false ||
                dbData.transactionInProgress
              ) {
            ...
            console.warn(
              `[Sync completed exchange products] Impossible de mettre à jour le produit ${prodId} à "sold":`,
              err,
            );
          }
        }
        ...
        if (!isFirstConvLoadRef.current) {
          snap.docChanges().forEach((change) => {
            if (change.type === "added" || change.type === "modified") {
              ...
              const isOurActiveChatNow =
                activeConversationIdRef.current === convData.id;

              if (
                convData.unreadUserId === user.uid &&
                !isMeSender &&
                (!isOurActiveChatNow || document.hidden)
              ) {
                ...
                setTimeout(() => {
                  setInAppNotification((prev) =>
                    prev?.body ===
                    (convData.lastMessage || "Vous avez reçu un message.")
                      ? null
                      : prev,
                  );
                }, 6000);
                ...
                if (
                  ...
                  new Notification(`Nouveau message de TrocShop`, {
                    body:
                      ...
                      "Discutons à propos de votre échange.",
                    icon: "/icon.jpg",
                    tag: convData.id,
                    renotify: true,
                  } as any);
  ...
  useEffect(() => {
    ...
    const pendingCount = exchanges.filter(
      (e) => e.status === "pending" && e.sellerId === user.uid,
    ).length;
  ...
  useEffect(() => {
    ...
    const qGlobal = query(collection(db, "global_notifications"), limit(10));
    ...
    const unsubscribeGlobal = onSnapshot(
      ...
      async (snap) => {
        ...
        try {
          syncedIds = JSON.parse(
            localStorage.getItem(`synced_global_${user.uid}`) || "[]",
          );
        } catch (e) {}
        ...
        if (hasChanges) {
          localStorage.setItem(
            `synced_global_${user.uid}`,
            JSON.stringify(newSyncedList),
          );
        }
      },
      (e) => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    let filtered = explorerProducts.filter((p) => {
      ...
      const matchCondition =
        filterCondition === "Tous" || p.condition === filterCondition;
      const matchNeighborhood =
        filterNeighborhood === "Tous" || p.neighborhood === filterNeighborhood;
      const matchListingType =
        filterListingType === "Tous" ||
      ...
      const seller =
        allUsersProfiles[p.sellerId] || globalUserProfileCache[p.sellerId];
  ...
  const filteredStudentProducts = useMemo(() => {
    ...
    const filtered = allUniqueProducts.filter((p) => {
      ...
      if (studentFilterListingType !== "Tous") {
        if (studentFilterListingType === "Vente" && p.listingType !== "sale")
          return false;
        if (studentFilterListingType === "Troc" && p.listingType !== "troc")
          return false;
        if (studentFilterListingType === "Troc +" && p.listingType !== "mixed")
          return false;
      }
  ...
  const filteredTrocProducts = useMemo(() => {
    const allUniqueProductsMap: Record<string, Product> = {};
    homeProducts.forEach((p) => {
      allUniqueProductsMap[p.id] = p;
    });
    ...
    const filtered = allUniqueProducts.filter((p) => {
      // Must be a Troc product by default (troc or mixed)
      const isTrocProduct =
        p.listingType === "troc" || p.listingType === "mixed";
      if (!isTrocProduct) return false;

      if (trocSearchQuery) {
        const query = trocSearchQuery.toLowerCase().trim();
        const title = (p.title || "").toLowerCase();
        const desc = (p.description || "").toLowerCase();
        const cat = (p.category || "").toLowerCase();
        if (
      ...
      if (trocFilterCondition !== "Tous") {
        if (p.condition !== trocFilterCondition) {
          return false;
        }
      }

      if (trocFilterPriceMax !== null) {
        if (p.price > trocFilterPriceMax) {
          return false;
        }
      }

      if (trocFilterListingType !== "Tous") {
        if (trocFilterListingType === "Troc" && p.listingType !== "troc")
          return false;
        if (trocFilterListingType === "Troc +" && p.listingType !== "mixed")
          return false;
      }

      if (trocFilterNeighborhood !== "Tous") {
        if (p.neighborhood !== trocFilterNeighborhood) return false;
      }
    ...
    return filtered.sort((a, b) => {
      if (trocSortBy === "price-asc") return a.price - b.price;
      if (trocSortBy === "price-desc") return b.price - a.price;
      const dateA = a.createdAt?.seconds || 0;
      const dateB = b.createdAt?.seconds || 0;
      if (trocSortBy === "oldest") return dateA - dateB;
      return dateB - dateA;
    });
  }, [
    homeProducts,
    ...
    trocFilterCondition,
    trocFilterPriceMax,
    trocFilterListingType,
    trocFilterNeighborhood,
    trocSortBy,
  ]);
  ...
  const filteredCategoryProducts = useMemo(() => {
    ...
    const filtered = allUniqueProducts.filter((p) => {
      ...
      if (categoryFilterListingType !== "Tous") {
        if (categoryFilterListingType === "Vente" && p.listingType !== "sale")
          return false;
        if (categoryFilterListingType === "Troc" && p.listingType !== "troc")
          return false;
        if (categoryFilterListingType === "Troc +" && p.listingType !== "mixed")
          return false;
      }
  ...
  const handleContactSellerFromSpotlight = async (
    sellerId: string,
    sellerNom: string,
    item: any,
  ) => {
    if (!user) {
      triggerAuthRequired(
        "Pour d'abord discuter avec le vendeur, vous devez d'abord vous connecter ou créer un compte.",
      );
      return;
    }
    ...
    if (sellerId === user.uid) {
      alert(
        "C'est votre profil ! Vous ne pouvez pas vous contacter vous-même.",
      );
      return;
    }

    const descriptionText = item?.description || "";
    const messageText = `Bonjour, je suis intéressé(e) par votre article vu sur TrocShop Shoplight : "${descriptionText}". Est-il toujours disponible ?`;

    safeConfirm(
      `Vous vous apprêtez à envoyer le message suivant à ${sellerNom || "vendeur"} :\n\n"${messageText}"\n\nVoulez-vous vraiment l'écrire et l'envoyer ?`,
      async () => {
        // Always create a spotlight-specific conversation thread for this video
        const convId =
          [user.uid, sellerId].sort().join("_") +
          "_spotlight_" +
          (item?.id || "default");
        ...
        try {
          ...
          const updateData = {
            productId: "spotlight",
            productTitle: `Shoplight`,
            productImage: "",
            productPrice: 0,
            lastMessage: messageText,
            lastSenderId: user.uid,
            updatedAt: serverTimestamp(),
            unreadUserId: sellerId,
          };
          ...
          if (!snap.exists()) {
            batch.set(convRef, {
              ...
              participantNames: [user.displayName || "Utilisateur", sellerNom],
              sellerId: sellerId,
              ...updateData,
            });
          } else {
          ...
          console.error("Error creating spotlight conversation:", err);
        }
      }
    );
  };

  const handleContactSellerDirectly = async (
    sellerId: string,
    sellerNom: string,
  ) => {
    if (!user) {
    ...
    const messageText = `Bonjour ${sellerNom || "vendeur"}, je vous contacte depuis votre profil public sur TrocShop.`;

    safeConfirm(
      `Vous vous apprêtez à envoyer le message suivant à ${sellerNom || "vendeur"} :\n\n"${messageText}"\n\nVoulez-vous vraiment l'écrire et l'envoyer ?`,
      async () => {
        const convId = [user.uid, sellerId].sort().join("_");
        const convRef = doc(db, "conversations", convId);
        ...
        try {
          ...
          const updateData = {
            productId: "direct_profile",
            productTitle: `Profil de ${sellerNom}`,
            productImage: "",
            productPrice: 0,
            lastMessage: messageText,
            lastSenderId: user.uid,
  ...
  const handleContactServiceSeller = async (
    sellerId: string,
    sellerName: string,
    service: any,
  ) => {
    if (!user) {
      triggerAuthRequired(
        "Pour d'abord discuter avec le prestataire, vous devez d'abord vous connecter ou créer un compte.",
      );
      return;
    }
    ...
    if (sellerId === user.uid) {
      alert(
        "C'est votre annonce de service ! Vous ne pouvez pas vous contacter vous-même.",
      );
      return;
    }

    const messageText = `Bonjour, je suis intéressé(e) par votre service : "${service?.title || ""}". Est-il toujours disponible ?`;

    safeConfirm(
      `Vous vous apprêtez à envoyer le message suivant à ${sellerName || "prestataire"} :\n\n"${messageText}"\n\nVoulez-vous vraiment l'écrire et l'envoyer ?`,
      async () => {
        const convId =
          [user.uid, sellerId].sort().join("_") + "_service_" + service.id;
        const convRef = doc(db, "conversations", convId);
        ...
        const productImage = service?.images?.[0] || "";
        ...
        try {
          ...
          if (!snap.exists()) {
            batch.set(convRef, {
              id: convId,
              participantIds: [user.uid, sellerId],
              participantNames: [user.displayName || "Utilisateur", sellerName],
              sellerId: sellerId,
              ...updateData,
            });
          } else {
          ...
          console.error("Error creating/updating service conversation:", err);
        }
      }
    );
  };

  const startConversation = async (p: Product, selectedImage?: string) => {
    if (!user) {
      triggerAuthRequired(
        "Pour discuter avec le vendeur, vous devez d'abord vous connecter ou créer un compte.",
      );
      return;
    }
    ...
    if (p.sellerId === user.uid) {
      alert(
        "C'est votre article ! Vous ne pouvez pas vous contacter vous-même.",
      );
      return;
    }

    const messageText = `Bonjour, je suis intéressé(e) par votre article : ${p.title}`;

    safeConfirm(
      `Vous vous apprêtez à envoyer le message suivant à ${p.sellerName || "vendeur"} :\n\n"${messageText}"\n\nVoulez-vous vraiment l'écrire et l'envoyer ?`,
      async () => {
        ...
        try {
          ...
          const updateData = {
            ...
            productImage: selectedImage || p.images[0],
          ...
          // Add exchange entry if not exists
  ...
  const handleProposeExchange = async (
    ...
    try {
      ...
      const exchangeTitles = exItems.map((p) => p.title).join(", ");
      let barterMsg = `🔄 PROPOSITION DE TROC\n\n` +
        `Bonjour ! Je suis intéressé par votre article :\n` +
        `• Demande : "${targetProduct.title}" (estimé à ${targetProduct.price.toLocaleString()} FCFA)\n\n` +
        `En échange, je vous propose mon/mes article(s) :\n` +
        `• Offre : "${exchangeTitles}"\n\n`;

      if (priceAdjustment > 0) {
        barterMsg += `💰 Avec une compensation financière de ma part de : +${priceAdjustment.toLocaleString()} FCFA\n\n`;
      } else if (priceAdjustment < 0) {
        barterMsg += `💰 Avec une demande de compensation financière de votre part de : ${Math.abs(priceAdjustment).toLocaleString()} FCFA\n\n`;
      }

      const convId = [user.uid, targetProduct.sellerId, targetProduct.id]
        .sort()
        .join("_");
      const convRef = doc(db, "conversations", convId);
      ...
      if (!snap.exists()) {
        batch.set(convRef, {
          id: convId,
          participantIds: [user.uid, targetProduct.sellerId],
          participantNames: [
            user.displayName || "Utilisateur",
            targetProduct.sellerName,
          ],
          productId: targetProduct.id,
          sellerId: targetProduct.sellerId,
          productTitle: targetProduct.title,
          productImage: targetProduct.images[0],
          productPrice: targetProduct.price,
          lastMessage: barterMsg,
          lastSenderId: user.uid,
          updatedAt: serverTimestamp(),
          unreadUserId: targetProduct.sellerId,
        });
      } else {
      ...
      // Add the pre-written barter message to the conversation
      const msgRef = doc(collection(convRef, "messages"));
      batch.set(msgRef, {
        senderId: user.uid,
        text: barterMsg,
        ...
        productId: targetProduct.id,
        exchangeProductId: myProductIds[0] || "",
        createdAt: serverTimestamp(),
      });

      // Notify recipient
      const notifRef = doc(
        collection(db, "users", targetProduct.sellerId, "notifications"),
      );
      batch.set(notifRef, {
        type: "message",
        title: `Proposition de Troc de ${user.displayName || "TrocShop"}`,
        body: barterMsg,
      ...
      // Open messages view immediately
      setActiveConversationId(convId);
      setActiveTab("messages");
    } catch (e) {
  ...
  const handleCompleteExchange = async (exId: string) => {
    ...
    try {
      ...
      if (
        exData.exchangeProductIds &&
        Array.isArray(exData.exchangeProductIds)
      ) {
      ...
      const notifSellerRef = doc(
        collection(db, "users", exData.sellerId, "notifications"),
      );
  ...
  const handleUpdateProductStatus = async (productId: string, status: ProductStatus) => {
    try {
      ...
      const labels: Record<string, string> = {
        sold: "L'article a été marqué comme vendu.",
        reserved: "L'article a été marqué comme réservé.",
        out_of_stock: "L'article a été marqué en rupture de stock.",
        available: "L'article est à nouveau actif et disponible.",
        paused: "L'annonce a été mise en pause.",
      };
      if (labels[status]) {
        alert(labels[status]);
      }
  ...
  const handleReserveProduct = async (productId: string) => {
    if (!user) {
      triggerAuthRequired("Pour réserver cet article, vous devez d'abord vous connecter ou créer un compte.");
      return;
    }

    safeConfirm(
      "Vous vous apprêtez à réserver l'article et vous ne pourrez pas annuler. Êtes-vous sûr ?",
      async () => {
        try {
          // 1. Call Supabase RPC as requested
          try {
            const { error } = await supabase.rpc('reserve_product', { p_product_id: productId });
            if (error) {
              console.warn("Supabase reserve_product RPC info:", error);
            }
          } catch (rpcErr) {
            console.warn("Supabase reserve_product RPC execution:", rpcErr);
          }

          const reservedUntil = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();

          // 2. Update Firestore & local state
          await updateDoc(doc(db, "products", productId), {
            status: "reserved",
            is_available: false,
          ...
          alert("Félicitations ! L'article a été réservé avec succès.");
        } catch (e) {
          console.error("Error reserving product:", e);
          handleFirestoreError(e, OperationType.UPDATE, "products");
        }
      }
    );
  ...
  const handleCancelReservation = async (productId: string) => {
    safeConfirm(
      "Êtes-vous sûr de vouloir annuler la réservation de cet article ? Il sera remis en vente.",
      async () => {
        try {
          // 1. Call Supabase RPC as requested
          try {
            ...
            if (error) {
              console.warn("Supabase cancel_reservation RPC info:", error);
            }
          } catch (rpcErr) {
            console.warn("Supabase cancel_reservation RPC execution:", rpcErr);
          }
          ...
          alert("La réservation a été annulée avec succès. L'article est de nouveau disponible.");
        } catch (e) {
  ...
  return (
    <div className="min-h-screen w-full bg-white antialiased">
      <div className="w-full min-h-screen min-[980px]:h-screen min-[980px]:overflow-hidden bg-white text-zinc-900 font-sans selection:bg-orange-100 selection:text-orange-900 relative flex flex-col overflow-x-hidden min-[980px]:flex-row min-[980px]:bg-white">
        {firestoreQuotaExceeded && (
          <div className="fixed md:absolute top-36 left-4 right-4 z-[99] max-w-lg mx-auto bg-orange-50/95 backdrop-blur-xl border border-orange-200/50 rounded-2xl p-3.5 shadow-lg flex items-start gap-3 animate-in fade-in slide-in-from-top-4 duration-300">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-black text-[11px] uppercase tracking-wider text-zinc-800">
                Base de données saturée
              </p>
              <p className="text-[10px] text-zinc-600 font-medium leading-relaxed mt-0.5">
                La limite quotidienne de requêtes à la base est atteinte.
                TrocShop continue de fonctionner en mode local sécurisé : vous
                pouvez naviguer grâce au cache local.
              </p>
            </div>
          </div>
        )}
        ...
        {/* Sidebar Desktop */}
        <aside className="hidden min-[980px]:flex fixed left-0 top-0 w-[220px] h-screen bg-white border-r border-zinc-200/80 flex-col justify-between p-6 z-[110] select-none">
          <div className="space-y-8">
            ...
            >
              <span className="text-2xl font-black italic tracking-tighter text-orange-600 select-none">
                TrocShop
              </span>
              <span className="px-1.5 py-0.5 text-[8px] font-black uppercase tracking-widest text-orange-600 bg-orange-100/80 border border-orange-200/50 rounded-md select-none transform -translate-y-1.5 font-sans leading-none">
                Bêta
              </span>
            </div>
            ...
            <nav className="flex flex-col gap-2">
              {[
                ...
                { id: "shoplight", label: "Shoplight", icon: <Tv size={20} /> },
                {
                  id: "messages",
                  label: "Messages",
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex items-center justify-between px-4 py-3 rounded-2xl text-xs font-bold uppercase tracking-wider transition-all",
                      isActive
                  ...
                  >
                    ...
                    {item.count !== undefined && item.count > 0 && (
                      <span className="bg-orange-600 text-white text-[10px] font-black px-1.5 py-0.5 rounded-full min-w-[1.25rem] h-5 flex items-center justify-center shadow-md shadow-orange-600/20">
                        {item.count}
                      </span>
                    )}
                  </button>
          ...
          {/* Bottom Section with + Publier button */}
          <div className="pt-4 border-t border-zinc-100">
            <button
              ...
              className="w-full bg-orange-600 text-white font-black uppercase text-[10px] tracking-widest py-4 rounded-2xl flex items-center justify-center gap-2 border border-orange-500 hover:bg-orange-700 transition-all shadow-lg shadow-orange-600/20 active:scale-95"
            >
              <Plus size={16} />
              <span>Publier</span>
            </button>
        ...
        {/* Watermark Background (Static for maximum performance) */}
        <div className="fixed md:absolute inset-0 flex items-center justify-center pointer-events-none select-none overflow-hidden z-0">
          <span className="text-[8rem] md:text-[14rem] font-black rotate-12 tracking-tighter text-orange-600/[0.03]">
            999
          </span>
        </div>
        ...
        <header
          className={cn(
            "fixed top-0 left-0 right-0 z-[1000] bg-white/95 backdrop-blur-xl border-b border-zinc-200/80 px-4 sm:px-6 h-16 sm:h-20 flex flex-col justify-center shadow-xs min-[980px]:fixed min-[980px]:top-0 min-[980px]:left-[220px] min-[980px]:right-0 min-[980px]:w-[calc(100%-220px)] min-[980px]:rounded-none min-[980px]:border-0 min-[980px]:border-b min-[980px]:border-zinc-200/80 min-[980px]:shadow-none min-[980px]:bg-white min-[980px]:px-8",
            activeTab === "shoplight" ||
              activeTab === "student" ||
              activeTab === "troc" ||
              activeTab === "category" ||
        ...
        >
          <div className="flex items-center justify-between w-full gap-4">
            ...
            {/* Section Title on Desktop left side */}
            <div className="hidden min-[980px]:block text-zinc-400 text-[10px] font-black uppercase tracking-widest select-none shrink-0 min-w-[120px]">
              {activeTab === "home"
                ? "Tableau de bord"
                : activeTab === "search"
                  ? "Recherche"
            ...
            <div className="hidden min-[980px]:flex flex-1 max-w-md mx-auto items-center relative">
              <Search className="absolute left-4 text-zinc-400 w-4 h-4 pointer-events-none" />
              <input
                type="text"
                placeholder="Rechercher sur TrocShop..."
                value={searchQuery}
                onChange={(e) => {
                ...
                className="w-full bg-zinc-50 border border-zinc-200 focus:border-orange-500 focus:ring-2 focus:ring-orange-500/10 rounded-full py-2.5 pl-11 pr-10 text-xs font-semibold placeholder:text-zinc-400 text-zinc-900 transition-all outline-none"
              />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery("")}
                  className="absolute right-3.5 p-1 text-zinc-400 hover:text-zinc-650 rounded-full hover:bg-zinc-100 transition-all"
                >
                  <X size={14} />
                </button>
              )}
                ...
                <div className="flex items-center gap-3">
                  ...
                  >
                    {profile?.photoURL ? (
                      <img
                        src={profile.photoURL}
                        className="w-10 h-10 rounded-xl border-2 border-white shadow-sm ring-1 ring-zinc-200 group-hover:ring-orange-500 transition-all object-cover"
                        alt="Avatar"
                      />
                    ) : (
        ...
        <main
          className={cn(
            activeTab === "shoplight"
              ? "w-full h-[100dvh] fixed inset-0 z-50 bg-black overflow-hidden min-[980px]:ml-[220px] min-[980px]:w-[calc(100%-220px)]"
              : activeTab === "student" ||
                  activeTab === "troc" ||
                  activeTab === "category" ||
                  activeTab === "services"
                ? "pt-4 min-[980px]:pt-6 pb-32 px-3 sm:px-6 max-w-lg mx-auto w-full h-auto md:h-full md:overflow-y-auto no-scrollbar relative transition-all duration-300 min-[980px]:max-w-none min-[980px]:mx-0 min-[980px]:ml-[220px] min-[980px]:w-[calc(100%-220px)] min-[980px]:px-12 min-[980px]:pb-12"
                : "pt-20 sm:pt-24 min-[980px]:pt-24 pb-32 px-3 sm:px-6 max-w-lg mx-auto w-full h-auto md:h-full md:overflow-y-auto no-scrollbar relative transition-all duration-300 min-[980px]:max-w-none min-[980px]:mx-0 min-[980px]:ml-[220px] min-[980px]:w-[calc(100%-220px)] min-[980px]:px-12 min-[980px]:pb-12",
          )}
        >
          <AnimatePresence mode="wait">
            ...
            {activeTab === "home" && (
              <div className="pt-1 pb-6 space-y-8">
                <DynamicBanner />

                {/* Espace Services Pill Block (Masqué comme demandé) */}
                {/* 
                <div className="px-1">
                  <button
                    onClick={() => {
                    ...
                    className="w-full p-4 rounded-[2rem] bg-indigo-50/70 border border-indigo-100/80 hover:bg-indigo-100/30 transition-all flex items-center justify-between group shadow-sm"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-2xl bg-indigo-500 flex items-center justify-center text-white text-xl">
                        💼
                      </div>
                      <div className="text-left">
                        <p className="text-[8px] font-black uppercase tracking-[0.25em] text-indigo-500">
                          Exclusif
                        </p>
                        <h4 className="font-black text-sm text-indigo-950 uppercase tracking-wider">
                          Espace Services
                        </h4>
                      </div>
                    </div>
                    <div className="w-8 h-8 rounded-full bg-white border border-indigo-100 flex items-center justify-center text-indigo-600 group-hover:translate-x-1 transition-transform shadow-sm">
                      <ArrowRight size={16} />
                    </div>
                  </button>
                </div>
                */}
                ...
                <div className="flex gap-3 overflow-x-auto py-2 no-scrollbar">
                  ...
                  <button
                    ...
                    className={cn(
                      "px-6 py-2 rounded-full border text-xs font-black uppercase tracking-widest transition-all shrink-0 flex items-center justify-center",
                      "bg-orange-50 border-orange-100 border-dashed text-orange-600 hover:bg-orange-100/40",
                    )}
                  >
                    <span>Troc</span>
                  </button>
                ...
                {/* Grid */}
                <div className="grid grid-cols-2 min-[980px]:grid-cols-3 min-[1440px]:grid-cols-4 gap-2.5 sm:gap-4">
                  {isInitialLoading ? (
                    [...Array(4)].map((_, i) => <ProductCardSkeleton key={i} />)
                  ) : homeProducts.length === 0 ? (
                    <div className="col-span-full py-20 text-center text-zinc-300">
            ...
            {activeTab === "search" && (
              <div className="py-6 space-y-8">
                  ...
                  <AnimatePresence>
                    {isFiltersOpen && (
                      <motion.div
                        ...
                        className="space-y-4 bg-zinc-50 p-6 rounded-[2rem] border border-zinc-100 shadow-inner overflow-visible"
                        style={{ overflow: "visible" }}
                      >
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-zinc-800">
                              Condition
                            </label>
                        ...
                        <div className="pt-2">
                          <button
                            onClick={() =>
                              setFilterCertifiedOnly(!filterCertifiedOnly)
                            }
                            className={cn(
                          ...
                          >
                            ...
                            />
                            {filterCertifiedOnly
                              ? "Comptes certifiés uniquement : ON"
                              : "Filtrer par comptes certifiés"}
                          </button>
                        </div>
                      </motion.div>
                    )}
                ...
                <div className="grid grid-cols-2 min-[980px]:grid-cols-3 min-[1440px]:grid-cols-4 gap-4">
                ...
                <div className="space-y-4">
                  <header className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h2 className="text-4xl font-black tracking-tighter uppercase italic text-emerald-600 font-sans">
                          Espace Étudiant
                        </h2>
                  ...
                  <div className="flex items-center gap-3 w-full">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setIsStudentFiltersOpen(!isStudentFiltersOpen)
                      }
                    >
                      <Filter size={20} />
                    </Button>
            ...
            {activeTab === "troc" && (
              <div className="py-6 space-y-8">
                <div className="space-y-4">
                  <header className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h2 className="text-4xl font-black tracking-tighter uppercase italic text-orange-600 font-sans">
                          Espace de Troc
                        </h2>
                        <div className="h-1.5 w-12 bg-orange-600 rounded-full mt-1" />
                      </div>
                  ...
                  {/* Banner Image with white background to blend in perfectly */}
                  <div className="w-full flex items-center justify-center bg-white rounded-[2.5rem] overflow-hidden mb-6 relative border border-zinc-100 shadow-sm p-4">
                    <img
                      src="/troc_banner.jpg"
                      alt="Espace de Troc"
                      className="max-h-[240px] w-auto object-contain bg-white"
                      referrerPolicy="no-referrer"
                    />
                  </div>
                  ...
                  <div className="flex items-center gap-3 w-full">
                    <div className="relative flex-1 group">
                      ...
                      <input
                        type="text"
                        value={trocSearchQuery}
                        onChange={(e) => setTrocSearchQuery(e.target.value)}
                        placeholder="Rechercher parmi les annonces de troc..."
                        className="w-full pl-12 pr-12 py-4 rounded-2xl bg-white border border-zinc-100 shadow-sm focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 outline-none transition-all font-medium text-sm"
                      />
                      {trocSearchQuery && (
                        <button
                          onClick={() => setTrocSearchQuery("")}
                          className="absolute right-4 top-1/2 -translate-y-1/2 p-2 bg-zinc-100 rounded-full text-zinc-400 hover:text-zinc-600 transition-all font-medium"
                        >
                          <X size={12} />
                        </button>
                    ...
                    <Button
                      ...
                      onClick={() => setIsTrocFiltersOpen(!isTrocFiltersOpen)}
                    >
                      <Filter size={20} />
                    </Button>
                  </div>
                    ...
                    {isTrocFiltersOpen && (
                      ...
                      >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-zinc-800">
                              Valeur Max (Ajustement)
                            </label>
                            <input
                              type="number"
                              placeholder="F CFA"
                              value={
                                trocFilterPriceMax !== null
                                  ? trocFilterPriceMax
                                  : ""
                              }
                              className="w-full bg-white border border-zinc-200 p-3 rounded-xl text-xs font-bold outline-none focus:border-orange-500"
                              onChange={(e) =>
                                setTrocFilterPriceMax(
                                  e.target.value
                                    ? Number(e.target.value)
                                    : null,
                                )
                          ...
                          <div className="space-y-2">
                            ...
                            <CustomDropdown
                              value={trocFilterNeighborhood}
                              options={["Tous", ...NEIGHBORHOODS]}
                              onChange={setTrocFilterNeighborhood}
                            />
                          </div>
                          ...
                          <div className="space-y-2 col-span-full">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {[
                                ...
                                <button
                                  key={c}
                                  onClick={() => setTrocFilterCondition(c)}
                                  className={cn(
                                    "px-3 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-wider transition-all border",
                                    trocFilterCondition === c
                          ...
                          <div className="space-y-2 col-span-full">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {["Tous", "Troc", "Troc +"].map((t) => (
                                <button
                                  key={t}
                                  onClick={() => setTrocFilterListingType(t)}
                                  className={cn(
                                    "px-3 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-wider transition-all border",
                                    trocFilterListingType === t
                                      ? "bg-orange-600 border-orange-600 text-white shadow-sm"
                                      : "bg-white text-zinc-500 border-zinc-200",
                                  )}
                                >
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar">
                    {[
                      { id: "newest", label: "Nouveautés" },
                      { id: "price-asc", label: "Estimation croissante" },
                      { id: "price-desc", label: "Estimation décroissante" },
                      { id: "oldest", label: "Ancienneté" },
                    ].map((sort) => (
                      <button
                        key={sort.id}
                        onClick={() => setTrocSortBy(sort.id as any)}
                        className={cn(
                          "px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all border",
                          trocSortBy === sort.id
                            ? "bg-zinc-900 text-white border-zinc-900 shadow-lg shadow-zinc-900/20"
                ...
                <div className="grid grid-cols-2 min-[980px]:grid-cols-3 min-[1440px]:grid-cols-4 gap-4">
                  ...
                  {filteredTrocProducts.length === 0 && (
                    <div className="col-span-full py-20 text-center text-zinc-400 font-bold uppercase tracking-widest text-xs">
                      <ShoppingBag className="w-16 h-16 mx-auto mb-4 opacity-10" />
                      Aucun article de troc correspondant
                    </div>
                  )}
                </div>
              </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-3 w-full">
                        <div className="relative flex-1 group">
                          <Search
                            className={cn(
                              "absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-zinc-300 transition-colors",
                              `group-focus-within:${meta.colorClass}`,
                            )}
                          />
                          <input
                            type="text"
                        ...
                        <Button
                          ...
                          onClick={() =>
                            setIsCategoryFiltersOpen(!isCategoryFiltersOpen)
                          }
                        >
                          <Filter size={20} />
                        </Button>
                          ...
                          >
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {["Tous", "Vente", "Troc", "Troc +"].map(
                                    (t) => (
                                      <button
                                        key={t}
                                        onClick={() =>
                                          setCategoryFilterListingType(t)
                                        }
                                        className={cn(
                                          "px-3 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-wider transition-all border",
                                          categoryFilterListingType === t
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {[
                                    ...
                                    <button
                                      key={c}
                                      onClick={() =>
                                        setCategoryFilterCondition(c)
                                      }
                                      className={cn(
                                        "px-3 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-wider transition-all border",
                                        categoryFilterCondition === c
                                          ? "bg-zinc-900 border-zinc-900 text-white shadow-sm"
                                          : "bg-white text-zinc-500 border-zinc-200",
                                      )}
                                    >
            ...
            {activeTab === "shoplight" && (
              <div className="absolute inset-0 w-full h-full bg-black">
                <Shoplight
                  ...
                  onContactSeller={handleContactSellerFromSpotlight}
                  onBackToHome={() => {
                    setActiveTab("home");
                    setActiveConversationId(null);
                    setSelectedSellerId(null);
            ...
            {/* Services Page (Masqué comme demandé) */}
            {/* 
            {activeTab === "services" && (
              <motion.div
                key="services"
              ...
              >
                <ServicesPage
                  onBackToHome={() => setActiveTab("home")}
                  user={user}
                  onContactSeller={handleContactServiceSeller}
                  onDetailToggle={(isOpen) => setIsNavHidden(isOpen)}
                  onSellerClick={(id) => {
                    if (!user) {
                      triggerAuthRequired(
                        "Pour visiter le profil d'un autre membre, vous devez d'abord vous connecter ou créer un compte.",
                      );
                      return;
                    }
            ...
            {activeTab === "sell" && (
              ...
              >
                <SellScreen
                  ...
                  setShowInAppCamera={setShowInAppCamera}
                />
              </motion.div>
            )}
                ...
                <div className="flex-1 overflow-hidden">
                  {activeConversationId ? (
                    <ChatWindow
                      ...
                      initialMessageText={chatInitialMessage}
                    ...
                    <div className="space-y-4 h-full overflow-y-auto no-scrollbar pb-32">
                      {conversations.length === 0 ? (
                        <div className="p-10 bg-white rounded-[2rem] border border-zinc-100 flex flex-col items-center text-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-zinc-400 max-w-[200px] mx-auto mt-1">
                              Trouvez un objet et commencez à troquer pour voir
                              vos discussions ici.
                            </p>
                          </div>
                          <Button
                        ...
                        conversations.map((conv) => {
                          const unreadCount =
                            unreadPerConversation[conv.id] || 0;
                            ...
                            <div key={conv.id} className="relative">
                              ...
                              <div
                                ...
                                className={cn(
                                  ...
                                  isSelected
                                    ? "ring-2 ring-orange-600 bg-orange-50 border-orange-200"
                                    : "border-zinc-100 shadow-sm hover:border-orange-200 hover:shadow-lg",
                                  isMessagesEditMode
                                    ? "pl-14 cursor-pointer"
                                    : "cursor-pointer",
                                )}
                              >
                                <AnimatePresence>
                                            ...
                                            <div className="flex-1 min-w-0">
                                              <h4 className="font-semibold text-lg truncate">
                                                {conv.participantNames?.find(
                                                  ...
                                                  `Utilisateur ${conv.participantIds?.find((id) => id !== user?.uid)?.slice(0, 4)}`}
                                              </h4>
                ...
                {!user ? (
                  <div className="py-20 text-center space-y-8">
                    ...
                    <div className="space-y-3">
                      ...
                      <p className="text-sm font-bold text-zinc-400 px-6 leading-relaxed uppercase tracking-widest text-[10px]">
                        Créez un compte pour vendre, troquer <br /> et discuter
                        avec la communauté.
                      </p>
                    </div>
                    <div className="flex flex-col gap-3 px-10">
                      <Button
                        variant="primary"
                        className="py-4 uppercase tracking-widest text-xs font-black"
                        onClick={() => {
                          setAuthMode("signup");
                          setActiveTab("auth");
                        }}
                      ...
                      <Button
                        variant="outline"
                        className="py-4 uppercase tracking-widest text-xs font-black border-zinc-200"
                        onClick={() => {
                          setAuthMode("login");
                          setActiveTab("auth");
                        }}
                  ...
                  <>
                      ...
                      <div className="h-32 bg-zinc-100 relative overflow-hidden">
                        {profile?.coverURL ? (
                          ...
                          <div className="w-full h-full bg-gradient-to-r from-orange-400/20 to-orange-600/20 flex items-center justify-center">
                            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-orange-600/40">
                              TrocShop Lounge
                            </span>
                          </div>
                        )}
                      ...
                      <div className="px-8 pb-8 -mt-14 relative z-10 space-y-6">
                        <div className="flex items-end justify-between">
                          {profile?.photoURL || user?.photoURL ? (
                            <img
                              ...
                              className="w-20 h-20 rounded-[2rem] border-4 border-white shadow-md object-cover relative bg-white"
                              alt="Me"
                            />
                          ) : (
                          ...
                          <div className="flex gap-1.5 sm:gap-2 flex-wrap">
                            <div
                              className="flex flex-col items-center px-3.5 py-2 bg-zinc-50 rounded-2xl cursor-pointer hover:bg-zinc-100 transition-colors"
                              onClick={() => setProfileView("listings")}
                            >
                        ...
                        <div className="flex items-center gap-4">
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-col items-start gap-1.5">
                              <h2 className="text-2xl font-black tracking-tighter leading-none truncate">
                                {user?.displayName}
                              </h2>
                              <div className="flex flex-col items-start gap-1.5">
                                {(() => {
                                    ...
                                    <>
                                      {isPartner && (
                                        <div className="bg-gradient-to-r from-emerald-500 to-green-600 px-3 py-1 rounded-xl text-[9px] font-black tracking-widest text-white shrink-0 shadow-md shadow-emerald-600/15 flex items-center gap-1 leading-none uppercase h-5">
                                          <Shield
                                            size={12}
                                            className="fill-white/20 select-none shrink-0"
                                          />
                                          Partenaire
                                        </div>
                                      )}
                            ...
                            <div className="flex items-center gap-3 mt-2">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-xs font-black">
                                  {privateRatingStats.count > 0
                                    ? privateRatingStats.rating
                                    : profile?.rating || 0}
                                </span>
                                <span className="text-[10px] text-zinc-400 font-bold">
                                  (
                                  {privateRatingStats.count > 0
                                    ? privateRatingStats.count
                        ...
                        {profile?.badges &&
                          filterUserBadges(
                            profile.badges,
                            profile.createdAt,
                          ).filter((b) => !b.toLowerCase().includes("certif"))
                            .length > 0 && (
                            <div className="space-y-2">
                              <h4 className="text-[8px] font-black uppercase tracking-[0.2em] text-zinc-400 ml-1">
                                Récompenses & Badges
                              </h4>
                              <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                                {filterUserBadges(
                                  ...
                                  .filter(
                                    (b) => !b.toLowerCase().includes("certif"),
                                  )
                                  .map((badge) => (
                                    <div
                                      key={badge}
                                      className="px-4 py-3 bg-zinc-50 text-[10px] font-black uppercase text-zinc-600 rounded-2xl border border-zinc-100 shadow-sm flex items-center gap-2 shrink-0"
                                    >
                        ...
                        <p className="text-zinc-500 text-sm leading-relaxed italic px-4 py-3 bg-zinc-50 rounded-2xl border border-zinc-100 border-dashed">
                          "{profile?.bio}"
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400 ml-2 mt-4">
                        Mon Espace
                      </h4>
                      <Button
                        variant="outline"
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="font-bold">Mon Réseau & Abonnements</span>
                            {(myFollowersCount > 0 || myFollowingCount > 0) && (
                              <span className="text-[10px] font-black px-2.5 py-0.5 bg-orange-100 text-orange-700 rounded-full">
                                {myFollowersCount + myFollowingCount}
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <Button
                        ...
                        className="w-full justify-between p-5 bg-white border-none shadow-sm cursor-pointer hover:bg-zinc-50"
                        onClick={async () => {
                          const profileShareUrl = `${window.location.origin}/?profile=${user?.uid}`;
                          const isCapacitor = !!(window as any).Capacitor;
                          if (isCapacitor) {
                            try {
                              await CapacitorShare.share({
                                title: "Mon Profil Public TrocShop",
                                text: `Découvrez mon profil public sur TrocShop !`,
                                url: profileShareUrl,
                          ...
                          if (navigator.share) {
                            try {
                              ...
                              const success = await safeCopyToClipboard(profileShareUrl);
                              if (success) {
                                toast.success("Lien de votre profil copié !");
                              } else {
                                toast.error("Impossible de copier le lien. Copiez : " + profileShareUrl);
                              }
                            }
                          } else {
                            const success = await safeCopyToClipboard(profileShareUrl);
                            if (success) {
                              toast.success("Lien de votre profil copié dans le presse-papiers !");
                            } else {
                              toast.error("Impossible de copier le lien. Copiez : " + profileShareUrl);
                            }
                          }
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-bold text-orange-600">Partager mon profil public</span>
                        </div>
                        <ChevronRight className="w-4 h-4 text-zinc-300" />
                      </Button>
                        ...
                        onClick={async () => {
                          ...
                          if (navigator.share) {
                            try {
                              ...
                              console.warn(
                                "navigator.share failed or blocked (e.g. inside iframe), falling back to clipboard:",
                                e,
                              );
                              const success =
                              ...
                              if (success) {
                                ...
                                toast.error(
                                  "Impossible de copier le lien automatiquement. Vous pouvez copier : " +
                                    shareUrl,
                                );
                              }
                            }
                      ...
                      <Button
                        ...
                        onClick={() => {
                          window.open(
                            "https://wa.me/2250160232164?text=Bonjour%20TrocShop,%20je%20souhaite%20signaler%20un%20bug%20ou%20sugg%C3%A9rer%20une%20am%C3%A9lioration%20:%20",
                            "_blank",
                          );
                        }}
                      >
                        <div className="flex items-center gap-3">
                          <span className="w-5 h-5 flex items-center justify-center shrink-0">
                            <svg
                              viewBox="0 0 24 24"
                              className="w-5 h-5 fill-[#25D366] text-[#25D366] shrink-0"
                      ...
                      <div className="p-5 bg-white rounded-3xl shadow-sm flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-xs font-black uppercase tracking-widest text-zinc-900 leading-tight">
                              Notifications
                            </p>
                          ...
                          onClick={async () => {
                            ...
                            if (user) {
                              try {
                                ...
                                setProfile((prev) =>
                                  prev
                                    ? { ...prev, notificationsEnabled: newVal }
                                    : null,
                                );
                              } catch (e: any) {
                                console.error(
                      ...
                      <Button
                        variant="ghost"
                        className="w-full text-red-500 hover:bg-red-50 flex items-center justify-center gap-2 mt-4 font-black uppercase text-[10px] tracking-widest py-4"
                  ...
                  <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                    <header className="flex items-center gap-4">
                      <button
                        ...
                        className="p-3 bg-white border border-zinc-100 rounded-2xl hover:bg-zinc-50 active:scale-95 transition-all"
                      >
                    ...
                    <div className="bg-white p-6 rounded-[2.5rem] border border-zinc-100 shadow-xl shadow-zinc-200/40 space-y-6 text-zinc-800">
                      <div className="flex items-center gap-3 border-b border-zinc-100 pb-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-black uppercase tracking-tight text-zinc-900">
                            Charte de confiance TrocShop
                          </p>
                        </div>
                      </div>

                      <p className="text-xs text-zinc-700 leading-relaxed font-bold">
                        Votre sécurité est notre priorité absolue. Pour éviter
                        les mauvaises surprises et faire de bonnes affaires en
                        toute confiance, voici les règles d’or à respecter sur
                        l’application.
                      </p>

                      <div className="space-y-4">
                        <h4 className="text-sm font-black text-zinc-900 uppercase tracking-tight flex items-center gap-2">
                          <span className="w-6 h-6 rounded-lg bg-orange-100 text-orange-600 text-xs font-black flex items-center justify-center">
                            1
                          </span>
                        ...
                        <div className="pl-8 space-y-4 text-xs text-zinc-650 leading-relaxed">
                          <p className="font-semibold">
                            Pour tous les vendeurs classiques de la ville,
                            appliquez strictement ces consignes :
                          </p>
                          ...
                          <div className="space-y-3">
                            <div className="flex gap-2.5 items-start">
                              ...
                              <div>
                                <strong className="text-zinc-900 font-extrabold uppercase text-[10px] tracking-wider block mb-0.5">
                                  Pas de paiement à l'avance
                                </strong>
                                Ne donnez jamais d'argent (ni espèces, ni Mobile
                                Money) avant d'avoir vu, touché et vérifié le
                                produit de vos propres yeux. Le paiement ou le
                                troc effectif se fait uniquement au moment de la
                            ...
                            <div className="flex gap-2.5 items-start">
                              ...
                              <div>
                                <strong className="text-zinc-900 font-extrabold uppercase text-[10px] tracking-wider block mb-0.5">
                                  Rendez-vous dans des lieux publics
                                </strong>
                                Ne fixez jamais de rendez-vous dans des endroits
                                isolés, sombres ou privés. Choisissez toujours
                                des milieux animés, sécurisés et très fréquentés
                                (campus de l'INPHB, grandes gares routières,
                                grands carrefours).
                              </div>
                            </div>
                            ...
                            <div className="flex gap-2.5 items-start">
                              ...
                              <div>
                                <strong className="text-zinc-900 font-extrabold uppercase text-[10px] tracking-wider block mb-0.5">
                                  Gardez vos informations secrètes
                                </strong>
                                Ne partagez jamais vos mots de passe, vos codes
                                confidentiels Mobile Money ou vos coordonnées
                                de carte bancaire. L'équipe TrocShop ne vous
                                demandera jamais un code confidentiel.
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="space-y-4 border-t border-zinc-150 pt-5">
                        ...
                        <div className="pl-8 space-y-3 text-xs text-zinc-650 leading-relaxed">
                          ...
                          <p>
                            Veuillez ne pas faire de paiement ou dépôt à l'avance à moins de connaître personnellement le vendeur. Dès lors, TrocShop se désengage de tout problème éventuel ou litige financier.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <ProfileSettings
                      ...
                      onSave={async (newData) => {
                        ...
                        try {
                          ...
                          alert(
                            "Impossible d'enregistrer les modifications du profil. Veuillez réessayer.",
                          );
                        }
                      }}
                    />
                  ...
                  <SellerListingsManager
                    ...
                    onDeleteProduct={(pId, e) => handleDeleteProduct(pId, e)}
                    onMarkAsSold={(pId) => handleMarkAsSold(pId)}
                    onUpdateStatus={(pId, st) => handleUpdateProductStatus(pId, st)}
                    onCancelReservation={(pId) => handleCancelReservation(pId)}
                  ...
                  <div className="space-y-6">
                    <header className="flex items-center gap-4">
                      <button
                        onClick={() => setProfileView("main")}
                        className="p-3 bg-white border border-zinc-100 rounded-2xl"
                      >
        ...
        <AnimatePresence>
          {showInstallBanner && (
            <motion.div
              ...
              className="fixed top-28 left-6 right-6 min-[980px]:top-24 z-[1001] bg-white text-zinc-900 p-4 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-zinc-150 flex items-center justify-between gap-3 max-w-lg mx-auto min-[980px]:left-[calc(220px+1.5rem)] min-[980px]:right-6"
            >
        ...
        {activeTab !== "auth" && (
          <nav
            className={cn(
              "fixed md:absolute bottom-[calc(0.75rem+env(safe-area-inset-bottom,12px))] md:bottom-6 left-3 right-3 sm:left-6 sm:right-6 z-[100] h-16 sm:h-20 transition-all duration-300 min-[980px]:hidden",
              activeConversationId ||
                isNavHidden ||
                activeTab === "notifications" ||
                activeTab === "shoplight"
          ...
          >
            <div className="h-full bg-white/95 backdrop-blur-2xl border border-zinc-200/80 rounded-[2rem] sm:rounded-[2.5rem] shadow-lg px-3 sm:px-6 flex items-center justify-around relative overflow-visible">
              ...
              <div
                ref={(el) => {
                  bottomNavItemRefs.current[0] = el;
                }}
              ...
              <div
                ref={(el) => {
                  bottomNavItemRefs.current[1] = el;
                }}
                className="relative z-20 flex-1 flex justify-center"
              >
                <NavButton
              ...
              <div
                ref={(el) => {
                  bottomNavItemRefs.current[2] = el;
                }}
                className="relative z-20 flex-1 flex justify-center"
              >
                <div className="relative">
                  <button
                    ...
                    className={cn(
                      ...
                      activeTab === "sell"
                        ? "scale-105 shadow-orange-600/80 border-orange-50"
                        : "",
                    )}
                  >
                    <Plus
              ...
              <div
                ref={(el) => {
                  bottomNavItemRefs.current[3] = el;
                }}
                className="relative z-20 flex-1 flex justify-center"
              >
                <NavButton
                  active={activeTab === "shoplight"}
                  onClick={() => {
                    setActiveTab("shoplight");
                    setActiveConversationId(null);
                    setSelectedSellerId(null);
              ...
              <div
                ref={(el) => {
                  bottomNavItemRefs.current[4] = el;
                }}
                className="relative z-20 flex-1 flex justify-center font-bold"
              >
                <NavButton
          ...
          {authRequiredMessage && (
            ...
            >
              ...
              >
                ...
                <div className="space-y-2">
                  <h3 className="font-black text-2xl tracking-tighter uppercase text-zinc-900 leading-none">
                    Connexion requise
                  </h3>
            ...
            >
              ...
              >
                ...
                <p className="text-zinc-600 text-sm font-semibold leading-relaxed">
                  ...
                  . Mettez-la à jour pour profiter de toutes les nouveautés !
                </p>
        ...
        <AnimatePresence>
          {selectedProduct && (
            <ProductSheet
              product={{
                ...selectedProduct,
                likesCount: getLikesCount(selectedProduct),
              }}
              isAlreadyInProfile={selectedSellerId === selectedProduct?.sellerId}
              onClose={() => setSelectedProduct(null)}
              user={user}
              onContact={(selectedImage) => startConversation(selectedProduct, selectedImage)}
              onExchange={() => {
                if (!user) {
                  triggerAuthRequired(
                    "Pour proposer un troc sur cet article, vous devez d'abord vous connecter ou créer un compte.",
                  );
                } else {
              ...
              onReserveProduct={(pId) => handleReserveProduct(pId)}
              onSellerClick={(id) => {
                if (!user) {
                  triggerAuthRequired(
                    "Pour visiter le profil d'un autre membre, vous devez d'abord vous connecter ou créer un compte.",
        ...
        <AnimatePresence>
          {showExchangePicker && selectedProduct && (
            <ExchangePicker
              ...
              onSelect={(ids, priceAdj) =>
                handleProposeExchange(selectedProduct, ids, priceAdj)
              }
              ...
              <div className="max-w-xl mx-auto px-6 py-12 pb-32">
                ...
                <div className="relative h-40 bg-gradient-to-tr from-orange-400 to-orange-500 rounded-[2.5rem] overflow-hidden mb-8 border border-zinc-100 shadow-lg">
                  {publicProfile.coverURL ? (
                    <img
                      src={publicProfile.coverURL}
                      className="w-full h-full object-cover"
                      alt="Couverture de boutique"
                    />
                  ) : (
                ...
                <div className="flex flex-col items-center text-center space-y-6 mb-12">
                  <div className="w-32 h-32 rounded-[2.5rem] bg-orange-100 flex items-center justify-center text-orange-600 text-4xl font-black italic shadow-2xl relative">
                    {publicProfile.photoURL ? (
                      ...
                      publicProfile.displayName[0]
                    )}
                    {(() => {
                      const isCert =
                        (publicProfile.is_certified as any) === true ||
                        (publicProfile.is_certified as any) === "vrai" ||
                        (publicProfile.isCertified as any) === true ||
                        (publicProfile.isCertified as any) === "vrai" ||
                        (publicProfile["is certified"] as any) === true ||
                        (publicProfile["is certified"] as any) === "vrai" ||
                        publicProfile.isPro === true ||
                        publicProfile.is_pro === true ||
                        (publicProfile.badges as string[])?.some((b) =>
                          b?.toLowerCase().includes("certif"),
                        );
                        ...
                        <>
                          {isCert && (
                            <div className="absolute -bottom-2 -right-2 bg-gradient-to-r from-amber-500 to-orange-600 rounded-full w-8 h-8 flex items-center justify-center text-white border-4 border-white shadow-md">
                              <BadgeCheck
                                size={16}
                                className="fill-white/20 select-none"
                              />
                            </div>
                          )}
                          {isVer && !isCert && (
                  ...
                  <div className="space-y-2 flex flex-col items-center">
                          ...
                          <>
                            ...
                            {publicProfile?.isStudent && (
                              <div className="bg-gradient-to-r from-emerald-500 to-teal-600 px-3 py-1 rounded-xl text-[9px] font-black tracking-widest text-white shrink-0 shadow-md shadow-emerald-600/15 flex items-center gap-1.5 leading-none uppercase h-5">
                                ...
                                <span>
                                  {publicProfile.studentSchool || "Campus"}
                                </span>
                              </div>
                            )}
                          </>
                    ...
                    <div className="flex items-center justify-center gap-4 text-zinc-400 font-bold text-xs uppercase tracking-widest">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-zinc-800 font-black">
                          {publicRatingStats.count > 0
                            ? publicRatingStats.rating
                            : publicProfile.rating || 0}
                        </span>
                        <span className="text-[10px] text-zinc-400 font-bold font-sans lowercase">
                          (
                          {publicRatingStats.count > 0
                            ? publicRatingStats.count
                            : publicProfile.reviewCount || 0}{" "}
                          avis)
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-center gap-6 py-2.5 px-6 bg-zinc-50 border border-zinc-150 rounded-2xl w-full max-w-xs shadow-sm my-1">
                      <div className="text-center">
                        <span className="block text-base font-black text-zinc-900 leading-none">
                  ...
                  <p className="text-zinc-500 font-medium max-w-md italic">
                    "{publicProfile.bio || "Passionné(e) de TrocShop !"}"
                  </p>
                  ...
                  <div className="flex gap-2">
                    {filterUserBadges(
                      ...
                      .map((badge) => (
                        <span
                          key={badge}
                          className="px-3 py-1 bg-zinc-900 text-white rounded-lg text-[8px] font-black uppercase tracking-widest"
                        >
                          {badge}
                        </span>
                      ))}
                  ...
                  <div className="flex flex-wrap pt-3 justify-center items-center gap-3 w-full">
                    {selectedSellerId !== user?.uid && (
                      <Button
                        onClick={handleToggleFollowSeller}
                        disabled={isFollowLoading}
                        className={cn(
                          "rounded-[1.5rem] px-6 py-3.5 text-[10px] uppercase font-black tracking-[0.15em] flex items-center gap-2 active:scale-95 transition-all cursor-pointer shadow-lg border-none min-w-[160px] justify-center",
                          isFollowingSeller
                            ? "bg-zinc-100 hover:bg-rose-50 hover:text-rose-600 text-zinc-800 border border-zinc-200 shadow-zinc-200/50"
                      ...
                      >
                        {isFollowLoading ? (
                          ...
                          <>
                            <UserCheck size={14} className="shrink-0 text-emerald-500" />
                            <span>Ne plus suivre</span>
                          </>
                        ) : (
                          <>
                            <UserPlus size={14} className="shrink-0 text-orange-400" />
                            <span>Suivre le vendeur</span>
                          </>
                        )}
                      </Button>
                    )}
                    ...
                    <Button
                      onClick={async () => {
                        const profileShareUrl = `${window.location.origin}/?profile=${selectedSellerId}`;
                        const isCapacitor = !!(window as any).Capacitor;
                        if (isCapacitor) {
                          try {
                            await CapacitorShare.share({
                              title: `Profil de ${publicProfile.displayName}`,
                              text: `Découvrez le profil de ${publicProfile.displayName} sur TrocShop !`,
                              url: profileShareUrl,
                      ...
                      className="bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white rounded-[1.5rem] px-6 py-3.5 text-[10px] uppercase font-black tracking-[0.15em] flex items-center gap-2.5 active:scale-95 transition-all cursor-pointer shadow-lg shadow-orange-500/15 hover:shadow-orange-500/25 border-none min-w-[160px] justify-center"
                    >
                      <Share size={14} className="shrink-0 text-white" />
                      <span>Partager</span>
                    </Button>
                  </div>
                ...
                {/* Advanced Search, Filtering, and Search Bar */}
                <div className="bg-zinc-50 border border-zinc-150 rounded-[2.5rem] p-5 mb-8 space-y-4">
                  <div className="relative">
                    <input
                      type="text"
                      ...
                      className="w-full pl-11 pr-16 py-3.5 rounded-[1.8rem] border border-zinc-200 bg-white text-xs font-semibold text-zinc-950 placeholder-zinc-400 focus:outline-none focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all shadow-sm"
                    />
                    <Search
                      size={14}
                      className="absolute left-4.5 top-1/2 -translate-y-1/2 text-zinc-400"
                    />
                    {publicSearchQuery && (
                      <button
                        type="button"
                        onClick={() => setPublicSearchQuery("")}
                        className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-black uppercase tracking-widest text-orange-600 hover:text-orange-700 bg-orange-50 px-2 py-1 rounded-md"
                      >
                        Effacer
                      </button>
                    )}
                  ...
                  <div className="flex items-center justify-between">
                    <button
                      type="button"
                      onClick={() =>
                        setPublicShowAdvancedFilters(!publicShowAdvancedFilters)
                      }
                      className={cn(
                        "flex items-center gap-1.5 px-4.5 py-2.5 rounded-[1.2rem] text-[10px] font-black uppercase tracking-widest transition-all shadow-sm",
                        publicShowAdvancedFilters
                    ...
                    >
                      ...
                      <ChevronDown
                        ...
                        className={cn(
                          ...
                          publicShowAdvancedFilters && "rotate-180",
                        )}
                      />
                    </button>
                    ...
                    {(publicSearchQuery ||
                      ...
                      <button
                        ...
                        className="text-[10px] font-black uppercase tracking-widest text-orange-600 hover:text-orange-700 bg-orange-50 px-3 py-1.5 rounded-xl transition-all active:scale-95"
                      >
                        Réinitialiser
                      </button>
                    )}
                  ...
                  <AnimatePresence>
                    {publicShowAdvancedFilters && (
                      <motion.div
                        ...
                        style={{ overflow: "visible" }}
                      >
                        <div className="pt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-zinc-150 mt-2">
                          <div className="space-y-1.5">
                            ...
                            <CustomDropdown
                              ...
                              options={[
                                ...
                                { value: "services_car", label: "Services : Auto/Moto" },
                                { value: "services_immo", label: "Services : Immo" },
                                */
                              ]}
                            />
                          </div>
                          ...
                          <div className="sm:col-span-2 space-y-1.5">
                            ...
                            <div className="flex items-center gap-2">
                              <input
                                type="number"
                                placeholder="Min"
                                value={publicMinPrice}
                                onChange={(e) =>
                                  setPublicMinPrice(e.target.value)
                                }
                                className="w-full px-3.5 py-2.5 rounded-xl border border-zinc-200 bg-white text-xs font-bold text-zinc-700 placeholder-zinc-400 focus:outline-none focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all"
                              />
                              ...
                              <input
                                type="number"
                                placeholder="Max"
                                value={publicMaxPrice}
                                onChange={(e) =>
                                  setPublicMaxPrice(e.target.value)
                                }
                                className="w-full px-3.5 py-2.5 rounded-xl border border-zinc-200 bg-white text-xs font-bold text-zinc-700 placeholder-zinc-400 focus:outline-none focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all"
                              />
                            </div>
                ...
                {(() => {
                  const isPublicProfilePro =
                    publicProfile.isPro === true ||
                    publicProfile.is_pro === true ||
                    (publicProfile.isPro as any) === "true" ||
                    (publicProfile.is_pro as any) === "true";
                  const filteredUserServices = allServices.filter(
                    (s) => s.sellerId === selectedSellerId,
                  );

                  // Apply filters & sort to sellerProducts
                  const filteredProducts = sellerProducts
                    .filter((p) => {
                      ...
                      const categoryMatch =
                        ...
                        p.category === publicCategoryFilter;

                      const priceNum = Number(p.price) || 0;
                      const minPriceMatch =
                        !publicMinPrice || priceNum >= Number(publicMinPrice);
                      const maxPriceMatch =
                        !publicMaxPrice || priceNum <= Number(publicMaxPrice);
                    ...
                    .sort((a, b) => {
                      if (publicSortBy === "price-asc") {
                        return (Number(a.price) || 0) - (Number(b.price) || 0);
                      }
                      if (publicSortBy === "price-desc") {
                  ...
                  // Apply filters & sort to filteredUserServices
                  const filteredServices = filteredUserServices
                    .filter((s) => {
                      const queryMatch =
                        !publicSearchQuery ||
                        s.title
                      ...
                      const categoryMatch =
                        !publicCategoryFilter ||
                        (publicCategoryFilter === "services_car" &&
                          s.serviceType === "car") ||
                        (publicCategoryFilter === "services_immo" &&
                          s.serviceType === "house") ||
                        // If it's another product category, it won't match any services
                        (!["services_car", "services_immo"].includes(
                          publicCategoryFilter,
                        ) &&
                          publicCategoryFilter !== "all");
                  ...
                  const showServicesTab = false; // isPublicProfilePro || filteredServices.length > 0; // Masqué comme demandé
                  const showShoplightTab =
                    publicSpotlights && publicSpotlights.length > 0;
                  const hasMultipleTabs = showServicesTab || showShoplightTab;

                  return (
                    <>
                      {hasMultipleTabs && (
                        <div className="bg-zinc-100 p-1 rounded-[2rem] flex gap-1 mb-8 max-w-sm mx-auto shadow-sm">
                          <button
                            type="button"
                            onClick={() => setPublicProfileTab("boutique")}
                            className={cn(
                              "flex-1 py-3 px-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all",
                              publicProfileTab === "boutique"
                                ? "bg-white text-zinc-900 shadow-sm"
                                : "text-zinc-500 hover:text-zinc-800",
                            )}
                          >
                            Boutique ({filteredProducts.length})
                          </button>
                          {showServicesTab && (
                            <button
                              type="button"
                              onClick={() => setPublicProfileTab("services")}
                              className={cn(
                                "flex-1 py-3 px-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all",
                                publicProfileTab === "services"
                                  ? "bg-white text-zinc-900 shadow-sm"
                                  : "text-zinc-500 hover:text-zinc-800",
                              )}
                            >
                          ...
                          {showShoplightTab && (
                            <button
                              type="button"
                              onClick={() => setPublicProfileTab("shoplight")}
                              className={cn(
                                "flex-1 py-3 px-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all",
                                publicProfileTab === "shoplight"
                                  ? "bg-white text-zinc-900 shadow-sm"
                                  : "text-zinc-500 hover:text-zinc-800",
                              )}
                            >
                              Shoplight ({publicSpotlights.length})
                            </button>
                          )}
                        </div>
                      )}
                      ...
                      {publicProfileTab === "services" && showServicesTab && (
                        <div className="space-y-8 animate-fadeIn">
                          <div className="flex items-center justify-between border-b-4 border-indigo-600 w-fit pb-2">
                            <h3 className="text-xl font-black italic uppercase tracking-tighter">
                              Services
                            </h3>
                          </div>
                          <div className="grid grid-cols-2 min-[980px]:grid-cols-3 min-[1440px]:grid-cols-4 gap-4">
                            {filteredServices.length > 0 ? (
                              filteredServices.map((s) => (
                                <div
                                  key={s.id}
                                  className="relative group bg-white rounded-3xl border border-zinc-150 overflow-hidden shadow-sm flex flex-col h-full cursor-pointer hover:shadow-md transition-all active:scale-[0.98]"
                                  onClick={() => {
                                    // Close public profile view
                                    setSelectedSellerId(null);
                                    // Open services tab and select the service
                                    setActiveTab("services");
                                    // Open service details
                                    (window as any).__openServiceDetailId =
                                      s.id;
                                    window.dispatchEvent(
                                      new CustomEvent("open-service-detail", {
                                        detail: s.id,
                                      }),
                                    );
                                  }}
                                >
                                  <div className="aspect-square w-full relative bg-zinc-50 flex items-center justify-center overflow-hidden">
                                    {s.images && s.images[0] ? (
                                      <img
                                        src={s.images[0]}
                                        alt={s.title}
                                      ...
                                      <div className="text-zinc-300 font-bold text-[10px] uppercase">
                                        Pas d'image
                                      </div>
                                    )}
                                    <div className="absolute top-3 right-3 bg-zinc-900/80 backdrop-blur-md px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest text-white">
                                      {s.serviceType === "car"
                                        ? "Auto/Moto"
                                        : "Immo"}
                                    </div>
                                  ...
                                  <div className="p-4 flex-1 flex flex-col justify-between space-y-2">
                                    <div>
                                      ...
                                      <h3 className="font-black text-xs text-zinc-900 leading-tight uppercase line-clamp-2">
                                        {s.title}
                                      </h3>
                                    </div>
                                    <div className="pt-2 border-t border-zinc-100 flex items-baseline gap-1">
                      ...
                      {publicProfileTab === "shoplight" && showShoplightTab && (
                        <div className="space-y-8 animate-fadeIn">
                          <div className="flex items-center justify-between border-b-4 border-orange-600 w-fit pb-2">
                            <h3 className="text-xl font-black italic uppercase tracking-tighter font-sans">
                              Shoplight
                            </h3>
                          </div>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 justify-center">
                            {publicSpotlights.map((item) => (
                              <div
                                key={item.id}
                                className="w-full max-w-sm mx-auto"
                              >
                                <PublicSpotlightPlayer
                                  item={item}
                                  sellerId={selectedSellerId || item.vendeur_id}
                                  sellerName={publicProfile?.displayName || item.vendeur_nom || "Vendeur"}
                                  onContactSeller={handleContactSellerFromSpotlight}
                                />
                              </div>
                            ))}
              ...
              >
                ...
                <div className="space-y-4">
                  <p className="text-xs text-zinc-650 font-extrabold leading-relaxed">
                    Pour une expérience sans mauvaise surprise, engagez-vous à
                    respecter nos 3 règles d'or :
                  </p>
                  ...
                  <div className="space-y-3 text-xs text-zinc-750 leading-relaxed pl-1">
                    <div className="flex gap-2.5 items-start">
                      ...
                      <p className="font-semibold">
                        ...
                        à moins de connaître personnellement le vendeur. Dès lors, TrocShop se désengage de tout problème éventuel.
                      </p>
                    </div>
                    ...
                    <div className="flex gap-2.5 items-start">
                      ...
                      <p className="font-semibold">
                        <strong className="text-zinc-900 font-extrabold">
                          Rencontres en public uniquement
                        </strong>{" "}
                        (Gare, grand carrefour, campus).
                      </p>
                    </div>
                    ...
                    <div className="flex gap-2.5 items-start">
                      ...
                      <p className="font-semibold">
                        ...
                        (Ne donnez jamais votre code de carte, Mobile Money ou
                        mot de passe dans le chat).
                      </p>
                    </div>
                  </div>
                </div>
          ...
          {inAppNotification && (
            ...
            >
              ...
              <div className="flex-1 min-w-0 text-left">
                <h4 className="font-extrabold text-xs uppercase tracking-wider text-orange-400 leading-none">
                  {inAppNotification.title}
                </h4>
                <p className="text-xs text-zinc-300 mt-1.5 truncate leading-tight font-medium">
              ...
              >
                <div
                  className={cn(
                    ...
                    customAlert &&
                      (customAlert.toLowerCase().includes("erreur") ||
                        customAlert.toLowerCase().includes("incorrect") ||
                        customAlert.toLowerCase().includes("impossible") ||
                        customAlert.toLowerCase().includes("obligatoire") ||
                        customAlert.toLowerCase().includes("déjà") ||
                        customAlert.toLowerCase().includes("incorrecte") ||
                        customAlert.toLowerCase().includes("pas de") ||
                        customAlert.toLowerCase().includes("🚨"))
                      ? "bg-rose-50 text-rose-600 animate-bounce"
                      : customAlert &&
                          (customAlert.toLowerCase().includes("succès") ||
                            customAlert.toLowerCase().includes("réussi") ||
                            customAlert.toLowerCase().includes("reçue") ||
                            customAlert.toLowerCase().includes("publiée"))
            ...
            >
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-white/80 font-medium max-w-xs mx-auto">
                    Votre proposition a été transmise à l'utilisateur. Patientez
                    jusqu'à sa décision.
                  </p>
                </div>
                <button
...
function SellScreen({
  ...
  showInAppCamera: boolean;
  setShowInAppCamera: (active: boolean) => void;
}) {
  const [publishContentType, setPublishContentType] = useState<'item' | 'video'>('item');
  const [isProUser, setIsProUser] = useState(false);
  const [pubType, setPubType] = useState<"item" | "service">(
    initialService ? "service" : "item",
  );

  // Helper to read temporary local draft from localStorage
  const getInitialDraft = React.useCallback(() => {
    if (initialProduct) return null;
    try {
      const saved = localStorage.getItem("trocshop_listing_draft_v1");
  ...
  const initialDraft = React.useMemo(() => getInitialDraft(), [getInitialDraft]);

  const [title, setTitle] = useState(() => {
    if (initialProduct) return initialProduct.title;
    if (initialDraft?.title) return initialDraft.title;
    try {
      return localStorage.getItem("sell_draft_title") || "";
    } catch {
      return "";
    }
  });
  const [desc, setDesc] = useState(() => {
    ...
    if (initialDraft?.desc) return initialDraft.desc;
    try {
      return localStorage.getItem("sell_draft_desc") || "";
    } catch {
      return "";
    }
  });
  const [price, setPrice] = useState(() => {
    ...
    if (initialDraft?.price) return initialDraft.price;
    try {
      return localStorage.getItem("sell_draft_price") || "";
    } catch {
      return "";
    }
  });
  const [category, setCategory] = useState(() => {
    ...
    if (initialDraft?.category) return initialDraft.category;
    try {
      return localStorage.getItem("sell_draft_category") || CATEGORIES[0];
    } catch {
      return CATEGORIES[0];
    }
  });
  const [neighborhood, setNeighborhood] = useState(() => {
    ...
    if (initialDraft?.neighborhood) return initialDraft.neighborhood;
    try {
      return localStorage.getItem("sell_draft_neighborhood") || "";
    } catch {
      return "";
    }
  });
  const [condition, setCondition] = useState(() => {
    ...
    if (initialDraft?.condition) return initialDraft.condition;
    try {
      return localStorage.getItem("sell_draft_condition") || "Comme neuf";
    } catch {
      return "Comme neuf";
    }
  });
  ...
  const [listingType, setListingType] = useState<"sale" | "troc" | "mixed">(
    () => {
      ...
      if (initialDraft?.listingType) return initialDraft.listingType;
      return "sale";
    },
  );
  const [exchangeWanted, setExchangeWanted] = useState(() => {
    if (initialProduct) return initialProduct.exchangeWanted || "";
    if (initialDraft?.exchangeWanted) return initialDraft.exchangeWanted;
    return "";
  });
  const [canTravel, setCanTravel] = useState(() => {
    if (initialProduct) return initialProduct.canTravel || false;
    if (typeof initialDraft?.canTravel === "boolean") return initialDraft.canTravel;
    return false;
  });
  const [meetingPoint, setMeetingPoint] = useState(() => {
    if (initialProduct) return initialProduct.meetingPoint || MEETING_POINTS[0];
    if (initialDraft?.meetingPoint) return initialDraft.meetingPoint;
    return MEETING_POINTS[0];
  });
  const [customMeetingPoint, setCustomMeetingPoint] = useState(() => {
    if (initialDraft?.customMeetingPoint) return initialDraft.customMeetingPoint;
    return "";
  });
  const [availability, setAvailability] = useState(() => {
    if (initialProduct)
      return initialProduct.availability || AVAILABILITY_OPTIONS[0];
    if (initialDraft?.availability) return initialDraft.availability;
  ...
  const [imagePreviews, setImagePreviews] = useState<string[]>(() => {
    ...
    if (Array.isArray(initialDraft?.imagePreviews)) return initialDraft.imagePreviews;
    try {
  ...
  const aiImproveDesc = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          systemInstruction:
            "Tu es un assistant qui génère exclusivement des listes à puces ultra-courtes pour des annonces. Chaque puce doit faire moins de 8 mots. Style direct.",
        },
      });
      if (response.text) {
      ...
      alert(
        "Impossible d'améliorer avec l'IA pour le moment. Réessayez plus tard.",
      );
    } finally {
  ...
  // BROUILLON EN COURS : Debounced auto-save (500ms) to localStorage
  useEffect(() => {
      ...
      try {
        const safeImages = Array.isArray(imagePreviews)
          ? imagePreviews.filter(
              (img) => typeof img === "string" && img.length < 2000000,
            )
          : [];
        const draft = {
          title,
          ...
          customMeetingPoint,
          availability,
          imagePreviews: safeImages,
        };
        localStorage.setItem("trocshop_listing_draft_v1", JSON.stringify(draft));
  ...
  const clearDraft = () => {
    try {
      localStorage.removeItem("trocshop_listing_draft_v1");
  ...
  // CHAMPS RÉCURRENTS : Load profiles.data.lastListingDefaults from server
  useEffect(() => {
    const fetchProfile = async () => {
      if (user) {
        ...
        if (p) {
          if (!initialProduct) {
            const defaults = p.lastListingDefaults || p.data?.lastListingDefaults;
            const hasDraft = !!localStorage.getItem("trocshop_listing_draft_v1");
            if (defaults) {
              // 1. Quartier
              const defQuartier = defaults.quartier || defaults.neighborhood;
              if (defQuartier && (!hasDraft || !neighborhood)) {
                setNeighborhood(defQuartier);
              } else if (!neighborhood && p.neighborhood && !hasDraft) {
                setNeighborhood(p.neighborhood);
              }

              // 2. Lieu de RDV
              const defRdv = defaults.lieuDeRdv || defaults.meetingPoint;
              if (defRdv && !hasDraft) {
                if (MEETING_POINTS.includes(defRdv)) {
                  setMeetingPoint(defRdv);
                } else {
                  setMeetingPoint("Personnalisé");
                  setCustomMeetingPoint(defRdv);
                }
              }

              // 3. Disponibilité
              const defDispo = defaults.disponibilite || defaults.availability;
              if (defDispo && !hasDraft) {
                if (AVAILABILITY_OPTIONS.includes(defDispo)) {
                  setAvailability(defDispo);
                }
              }

              // 4. Mobilite / canTravel
              if (typeof defaults.canTravel === "boolean" && !hasDraft) {
                setCanTravel(defaults.canTravel);
              }
            } else if (p.neighborhood && !neighborhood && !hasDraft) {
              setNeighborhood(p.neighborhood);
            }
          }
      ...
      <div className="py-20 text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-zinc-500 font-black uppercase tracking-widest leading-relaxed">
            Connectez-vous pour commencer <br /> à vendre sur TrocShop.
          </p>
        </div>
        <Button
          onClick={onComplete}
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (imagePreviews.length + files.length > 6) {
      alert(
        "La publication d'une annonce requiert au maximum 6 photos. Pas plus de 6 photos.",
      );
      return;
    }
    ...
    try {
      ...
      alert(
        "Désolé, impossible de traiter cette image. Elle est peut-être trop lourde.",
      );
    } finally {
  ...
  const startCamera = async () => {
    if (imagePreviews.length >= 6) {
      alert(
        "La publication d'une annonce requiert au maximum 6 photos. Pas plus.",
      );
      return;
    }
    try {
      ...
      console.warn(
        "Browser camera denied/unavailable, defaulting to device capture:",
        err,
      );
  ...
  const capturePhoto = async () => {
    if (imagePreviews.length >= 6) {
      alert("La publication d'une annonce requiert au maximum 6 photos.");
      stopCamera();
      return;
    }
    if (videoRef.current) {
      try {
        ...
        if (ctx) {
          ...
          setImagePreviews((prev) => {
            ...
            if (updated.length >= 6) {
              setTimeout(() => {
                alert("Vous avez pris 6 photos (maximum atteint).");
                stopCamera();
              }, 300);
            }
            return updated;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (imagePreviews.length < 4 || imagePreviews.length > 6) {
      alert(
        "La publication d'une annonce requiert entre 4 et 6 photos. (Minimum: 4, Maximum: 6)",
      );
      return;
    }
    ...
    if (listingType !== "troc") {
      ...
      if (!price || isNaN(parsedPrice) || parsedPrice <= 0) {
        alert(
          "L'ajout du prix est obligatoire. Seuls les trocs simples peuvent être d'un prix optionnel.",
        );
        return;
      }
    }
    ...
    const finalMeetingPoint =
      meetingPoint === "Personnalisé"
        ? customMeetingPoint.trim()
        : meetingPoint;

    setPublishStatus("Vérification des informations...");
    setLoading(true);
    try {
      const productRef = initialProduct
      ...
      setPublishStatus(
        `Optimisation et upload des ${imagePreviews.length} photos...`,
      );
      ...
      try {
        ...
        console.warn(
          "Storage upload warning during product publish/update, falling back to local storage URLs:",
          uploadError,
        );
      ...
      setPublishStatus(
        "Envoi des données de l'annonce à la base de données...",
      );
      if (initialProduct) {
        ...
        // CHAMPS RÉCURRENTS : Enregistre lastListingDefaults dans profiles.data
        if (user?.uid) {
          const defaultsToSave = {
            quartier: neighborhood || "",
            lieuDeRdv: finalMeetingPoint || meetingPoint || "",
            disponibilite: availability || "",
            canTravel: Boolean(canTravel),
          };
          try {
            await updateDoc(doc(db, "users", user.uid), {
              lastListingDefaults: defaultsToSave,
              "data.lastListingDefaults": defaultsToSave,
            }).catch(async () => {
              await setDoc(
            ...
            if (globalUserProfileCache[user.uid]) {
              globalUserProfileCache[user.uid].lastListingDefaults = defaultsToSave;
            }
            if (globalDataCache[`userProfile_${user.uid}`]) {
              globalDataCache[`userProfile_${user.uid}`].lastListingDefaults = defaultsToSave;
            }
          } catch (errProfile) {
            console.warn("Error saving lastListingDefaults to profile:", errProfile);
          }
        }
      ...
      let errMsg =
        "Désolé, une erreur s'est produite lors de l'enregistrement de votre annonce.";
      ...
      if (
        ...
        errMsg =
          "Erreur de permissions Firebase : Vous n'êtes pas autorisé à modifier ou publier des articles dans cette base de données.";
      } else if (
        errStr.includes("quota") ||
        errStr.includes("resource-exhausted")
      ) {
        errMsg =
          "Quota Firebase dépassé : Plus aucun nouvel article ne peut être créé car la limite quotidienne gratuite du serveur est atteinte.";
      } else if (errStr.includes("offline") || errStr.includes("offline")) {
        errMsg =
          "Vous êtes actuellement hors ligne ou votre connexion est trop faible.";
      }
      alert(errMsg);
      toast.error(errMsg);
    } finally {
      ...
      {/* (Masqué comme demandé)
      if (pubType === "service" && !initialProduct) {
        return (
          <div className="space-y-6 max-w-sm mx-auto">
              ...
              <div className="flex justify-between items-center">
                <div>
                  <h2 className="text-4xl font-black tracking-tighter uppercase italic leading-none">
                    {initialService ? "MODIFIER" : "PUBLIER"}
                  </h2>
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-400">
                    {initialService
                      ? "Modifiez les informations de votre service"
                      : "Postez votre service en 30 secondes"}
                  </p>
                </div>
              </div>

              {!initialService && (
                <div className="bg-zinc-100 p-1 rounded-[2rem] flex gap-1 mt-4">
                  <button
                    type="button"
                    onClick={() => setPubType("item")}
                    className="flex-1 py-4 px-2 rounded-[1.8rem] text-[10px] font-black uppercase tracking-widest transition-all text-zinc-400 hover:text-zinc-600"
                  >
                    Articles
                  </button>
                  <button
                    type="button"
                    onClick={() => setPubType("service")}
                    className="flex-1 py-4 px-2 rounded-[1.8rem] text-[10px] font-black uppercase tracking-widest transition-all bg-indigo-600 text-white shadow-sm"
                  >
                    Services
                  </button>
                </div>
            ...
            {isProUser ? (
              ...
              <div className="bg-white border border-zinc-150 rounded-[2.5rem] p-8 text-center space-y-5 shadow-sm">
                <div className="w-14 h-14 bg-indigo-50 rounded-full flex items-center justify-center mx-auto text-indigo-600">
                  <ShieldAlert size={28} />
                </div>
                <div className="space-y-2">
                  <h3 className="font-black text-sm uppercase tracking-tight text-zinc-900">
                    Accès Services Réservé
                  </h3>
                  <p className="text-xs text-zinc-500 leading-relaxed font-medium">
                    La publication de services est une fonctionnalité exclusive
                    réservée aux comptes professionnels certifiés sur TrocShop.
                  </p>
                </div>
                <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider leading-normal">
                  Votre compte actuel n'est pas configuré comme professionnel.
                </p>
                <a
                  href="https://wa.me/2250160232164?text=Bonjour%20TrocShop,%20je%20souhaite%20activer%20l'accès%20professionnel%20pour%20mon%20compte%20pour%20publier%20des%20services."
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex w-full justify-center items-center gap-2 p-4 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl shadow-md hover:bg-indigo-700 active:scale-95 transition-all"
                >
  ...
  return (
    <div className="space-y-6 max-w-sm sm:max-w-md mx-auto">
      {/* Switch Pill (Annonce vs Vidéo) */}
      {!initialProduct && (
        <div className="bg-zinc-100 p-1.5 rounded-full flex items-center border border-zinc-200/90 shadow-2xs">
          <button
            type="button"
            onClick={() => setPublishContentType('item')}
            className={cn(
              "flex-1 py-2.5 px-3.5 rounded-full text-xs font-black uppercase tracking-wider transition-all duration-200 flex items-center justify-center cursor-pointer",
              publishContentType === 'item'
          ...
          <button
            type="button"
            onClick={() => setPublishContentType('video')}
            className={cn(
              "flex-1 py-2.5 px-3.5 rounded-full text-xs font-black uppercase tracking-wider transition-all duration-200 flex items-center justify-center cursor-pointer",
          ...
          >
            <span>Vidéo Shoplight</span>
          </button>
        </div>
      )}

      {publishContentType === 'video' && !initialProduct ? (
        user ? (
          <SpotlightPublisher
            user={user}
            mode="publish"
            onVideoPublished={() => {
              alert("La vidéo a bien été publiée !");
              setPublishContentType('item');
              onComplete();
            }}
          />
        ) : (
          <div className="p-6 bg-amber-50 border border-amber-200 rounded-2xl text-amber-800 text-xs font-semibold text-center">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-400">
                  Postez votre annonce en 30 secondes
                </p>
              </div>
            </div>

        {/* Toggle Articles / Services (Masqué comme demandé) */}
        {/* 
        {!initialProduct && (
          <div className="bg-zinc-100 p-1 rounded-[2rem] flex gap-1 mt-4">
            <button
        ...
        <div className="space-y-3">
          <label className="text-[10px] font-black uppercase text-zinc-800 tracking-[0.2em] ml-1 flex items-center justify-between">
            ...
            <span className="text-[8px] font-black text-rose-600">
              Entre 4 et 6 photos obligatoires
            </span>
          </label>
          ...
          <div className="space-y-4">
            <div className="flex gap-3">
              ...
              <button
                type="button"
                onClick={startCamera}
                className="flex-1 h-14 bg-orange-50 border border-orange-200 rounded-2xl flex items-center justify-center gap-2 text-orange-600 hover:bg-orange-100 transition-all active:scale-95 group cursor-pointer shadow-xs"
              >
            ...
            {/* In-App Camera Viewport - Supports taking multiple photos sequentially */}
            {showInAppCamera && (
              <div className="fixed inset-0 bg-black/95 z-[999] flex flex-col justify-between p-4 sm:p-6 select-none animate-in fade-in duration-150">
                <div className="flex justify-between items-center z-10">
                  <div>
                    <h3 className="text-white text-xs font-black uppercase tracking-widest flex items-center gap-2">
                      <Camera size={16} className="text-orange-500" />
                      Appareil Photo TrocShop
                    </h3>
                    <p className="text-[10px] font-bold text-zinc-400">
                      {imagePreviews.length}/6 photo{imagePreviews.length > 1 ? "s" : ""} prise{imagePreviews.length > 1 ? "s" : ""}
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={stopCamera}
                    className="px-3.5 py-1.5 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white text-xs font-bold active:scale-95 transition-all cursor-pointer"
                  >
                    Fermer
                  </button>
                </div>

                <div className="flex-1 my-3 flex items-center justify-center relative overflow-hidden rounded-[2rem] border border-white/10 bg-zinc-950">
                  <video
                    ref={videoRef}
                    autoPlay
                    playsInline
                  ...
                  />
                  {photoFlash && (
                    <div className="absolute inset-0 bg-white pointer-events-none animate-pulse z-20" />
                  )}
                  <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-black/70 backdrop-blur-md px-3.5 py-1.5 rounded-full text-white text-[10px] font-bold border border-white/20 shadow-md text-center">
                    Répétez les prises de vue pour ajouter plusieurs photos
                  </div>
                </div>

                {/* Strip of taken photos inside camera */}
                {imagePreviews.length > 0 && (
                  <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-2 mb-2 z-10">
                    {imagePreviews.map((p, idx) => (
                      <div
                        key={idx}
                        className="relative h-14 w-14 shrink-0 rounded-xl overflow-hidden border-2 border-orange-500 shadow-md"
                      >
                        <img src={p} className="w-full h-full object-cover" alt={`Photo ${idx + 1}`} />
                        <span className="absolute bottom-0.5 right-0.5 bg-black/80 text-white text-[9px] font-extrabold px-1 rounded">
                          #{idx + 1}
                        </span>
                      </div>
                    ))}
                ...
                <div className="flex items-center justify-between px-2 pb-2 z-10">
                  ...
                  <button
                    ...
                    className="w-20 h-20 bg-white rounded-full p-1.5 active:scale-90 transition-transform shadow-2xl disabled:opacity-50 cursor-pointer"
                  ...
                  <div className="w-20 flex justify-end">
                    {imagePreviews.length > 0 && (
                      <button
                        type="button"
                        onClick={stopCamera}
                        className="px-3 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-xs font-black uppercase tracking-wider flex items-center gap-1 shadow-lg active:scale-95 transition-all cursor-pointer"
                      >
                        <Check size={14} />
                        Terminer
                      </button>
            ...
            {/* Thumbnail previews grid & click to enlarge */}
            {imagePreviews.length > 0 && (
              <div className="space-y-1.5">
                ...
                <div className="flex gap-3 overflow-x-auto no-scrollbar py-2 px-1">
                  {imagePreviews.map((p, i) => (
                    <div
                      key={i}
                      onClick={() => setEnlargedPhotoIndex(i)}
                      className="aspect-square h-24 w-24 shrink-0 relative rounded-xl overflow-hidden border-2 border-zinc-200 group shadow-xs transition-all hover:border-orange-500 cursor-pointer"
                    >
                      <img
                        src={p}
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/25 transition-colors flex items-center justify-center">
                        <Maximize2 size={18} className="text-white opacity-0 group-hover:opacity-100 drop-shadow-md transition-opacity" />
                      </div>
                      <span className="absolute bottom-1 left-1 bg-black/70 text-white text-[9px] font-black px-1.5 py-0.5 rounded-md backdrop-blur-xs">
                        #{i + 1}
                      </span>
                      <button
                        type="button"
                        ...
                        className="absolute top-1 right-1 w-6 h-6 bg-rose-600 hover:bg-rose-700 text-white rounded-full flex items-center justify-center text-xs font-black shadow-md z-10 cursor-pointer transition-transform hover:scale-110"
            ...
            {/* Enlarged Fullscreen Photo Modal */}
            {enlargedPhotoIndex !== null && imagePreviews[enlargedPhotoIndex] && (
              <div className="fixed inset-0 z-[1000] bg-black/95 backdrop-blur-md flex flex-col justify-between p-4 sm:p-6 select-none animate-in fade-in duration-200">
                <div className="flex justify-between items-center text-white z-10">
                  <span className="text-xs font-black uppercase tracking-widest bg-zinc-800/90 px-3.5 py-1.5 rounded-full border border-white/10 shadow-sm">
                    Photo {enlargedPhotoIndex + 1} sur {imagePreviews.length}
                  </span>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={() => {
                        const idxToRemove = enlargedPhotoIndex;
                        removeImage(idxToRemove);
                        if (imagePreviews.length <= 1) {
                          setEnlargedPhotoIndex(null);
                        } else if (idxToRemove >= imagePreviews.length - 1) {
                          setEnlargedPhotoIndex(imagePreviews.length - 2);
                        }
                      }}
                      className="px-3.5 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-full text-xs font-black flex items-center gap-1.5 transition-colors cursor-pointer shadow-md"
                    >
                      <Trash2 size={14} />
                      <span>Supprimer</span>
                    </button>
                    ...
                    <button
                      ...
                      className="w-9 h-9 bg-white/20 hover:bg-white/30 text-white rounded-full flex items-center justify-center font-black transition-colors cursor-pointer"
                      title="Fermer"
                    >
                      <X size={18} />
                    </button>
                ...
                <div className="relative flex-1 my-4 flex items-center justify-center">
                    ...
                    <button
                      ...
                      onClick={() =>
                        setEnlargedPhotoIndex((prev) =>
                          prev === null || prev === 0 ? imagePreviews.length - 1 : prev - 1
                        )
                      }
                      className="absolute left-1 sm:left-4 z-20 w-11 h-11 bg-black/70 hover:bg-black text-white rounded-full flex items-center justify-center backdrop-blur-md transition-all cursor-pointer shadow-xl border border-white/20 active:scale-95"
                  ...
                  <img
                    src={imagePreviews[enlargedPhotoIndex]}
                    alt={`Aperçu photo ${enlargedPhotoIndex + 1}`}
                  ...
                  {imagePreviews.length > 1 && (
                    <button
                      ...
                      onClick={() =>
                        setEnlargedPhotoIndex((prev) =>
                          prev === null || prev === imagePreviews.length - 1 ? 0 : prev + 1
                        )
                      }
                      className="absolute right-1 sm:left-4 z-20 w-11 h-11 bg-black/70 hover:bg-black text-white rounded-full flex items-center justify-center backdrop-blur-md transition-all cursor-pointer shadow-xl border border-white/20 active:scale-95"
                ...
                {imagePreviews.length > 1 && (
                  <div className="flex justify-center gap-2 overflow-x-auto no-scrollbar py-2 z-10">
                    {imagePreviews.map((img, idx) => (
                      <button
                        ...
                        className={cn(
                          "w-12 h-12 shrink-0 rounded-xl overflow-hidden border-2 transition-all cursor-pointer",
          ...
          {compressing && (
            <div className="flex items-center gap-2 px-2 py-1">
              ...
              <span className="text-[8px] font-black uppercase text-orange-600 tracking-widest">
                Traitement...
              </span>
            </div>
          )}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              {listingType !== "troc" && (
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-zinc-800 tracking-widest ml-1">
                    {listingType === "mixed"
                      ? "Somme que vous ajoutez (Obligatoire)"
                      : "Prix (CFA)"}
                  </label>
                  <div className="relative">
                    <input
                      ...
                      required
                    />
                    <span className="absolute right-6 top-1/2 -translate-y-1/2 text-zinc-300 font-bold text-xs uppercase tracking-widest pointer-events-none">
                      F CFA
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="Ex: PC, SMARTPHONE, MOTO..."
                      required
                    />
                    <Handshake
                      className="absolute right-6 top-1/2 -translate-y-1/2 text-zinc-200"
                      size={20}
                    />
                  </div>
                </div>
                  ...
                  <div className="mt-3 space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                    <div className="bg-orange-50 text-orange-800 text-[10px] p-3 rounded-xl flex items-start gap-2">
                      ...
                      <p className="leading-tight font-medium">
                        Pour votre sécurité, veuillez choisir un lieu public et
                        fréquenté pour effectuer vos trocs (ex: Supermarché,
                        Place publique).
                      </p>
                    </div>
                    <input
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                placeholder="Détaillez l'état, les défauts, les accessoires inclus..."
                required
              />
            </div>
          </div>
        ...
        <div className="flex flex-col gap-3 pt-6">
          ...
          {loading && publishStatus && (
            <div className="p-4 bg-orange-50/80 rounded-2xl border border-orange-100 flex items-center gap-3 animate-pulse">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-orange-800 leading-none">
                {publishStatus}
              </p>
            </div>
          )}
...
function ExchangePicker({
  ...
  myProducts: Product[];
  onSelect: (ids: string[], priceAdjustment: number) => void;
  onClose: () => void;
  targetProduct: Product;
}) {
        ...
        <header className="p-10 border-b border-zinc-100 bg-zinc-50/50">
          <div className="flex items-center gap-6 mb-8">
            <div className="flex-1">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mt-2 ml-8">
                {targetProduct.listingType === "mixed"
                  ? "Sélectionnez votre article à échanger"
                  : "Sélectionnez vos articles & ajustement"}
              </p>
              <div className="mt-4 ml-8">
                <p className="text-lg font-black text-zinc-900 truncate uppercase tracking-tighter">
                  {targetProduct.title}
                </p>
                <p className="text-sm font-black text-orange-600 italic">
                  {formatPrice(targetProduct.price)} FCFA
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4 ml-8">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  />
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-300 font-bold text-[10px] uppercase tracking-widest pointer-events-none">
                    F CFA
                  </span>
                </div>
              </div>
          ...
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-black uppercase tracking-widest text-zinc-900">
              Vos articles ({selectedIds.length})
            </h4>
            <button
              onClick={onClose}
              className="p-3 bg-white shadow-sm rounded-xl hover:bg-zinc-100 transition-colors"
            >
              <X size={20} />
            </button>
          </div>
            ...
            <div className="py-20 text-center space-y-8">
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs text-zinc-400 font-medium max-w-xs mx-auto">
                  Vous devez avoir au moins une annonce en ligne pour proposer
                  un échange.
                </p>
              </div>
              <Button
                variant="primary"
                onClick={onClose}
                className="mx-auto rounded-full py-4 px-8 uppercase text-[10px] font-black tracking-widest"
              >
                Retour
              </Button>
            </div>
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    ...
                    isSelected
                      ? "border-orange-600 bg-white ring-4 ring-orange-100"
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                    <h4 className="font-black uppercase tracking-tight text-lg truncate group-hover:text-orange-600 transition-colors">
                      {p.title}
                    </h4>
                    <div className="flex items-center gap-2">
...
function ProductSheet({
  ...
  onClose: () => void;
  user: User | null;
  onContact: (selectedImage?: string) => void;
  onExchange?: () => void;
  favorite?: boolean;
  onFavorite?: () => void;
  ...
  return (
    <>
      <AnimatePresence>
        {zoomedImage && (
          <motion.div
            ...
            className="fixed md:absolute top-0 right-0 bottom-0 left-0 min-[980px]:left-[220px] z-[110] bg-black/95 flex items-center justify-center p-4 cursor-default"
          >
            <button
              onClick={(e) => {
                e.stopPropagation();
      ...
      <div
        className="fixed md:absolute top-0 right-0 bottom-0 left-0 min-[980px]:left-[220px] z-[100] bg-black/60 backdrop-blur-md flex items-end md:items-center justify-center p-0 md:p-6"
        onClick={onClose}
      >
        <div
          className="w-full h-full md:max-w-2xl md:h-[92vh] md:rounded-[3rem] md:shadow-2xl md:border md:border-zinc-100/50 bg-white rounded-t-[3rem] overflow-hidden flex flex-col relative"
          onClick={(e) => e.stopPropagation()}
        >
            ...
            <div className="p-8 md:p-14 space-y-12">
              <div className="space-y-6">
                <div className="flex flex-wrap gap-2">
                  ...
                  <span className="px-4 py-1.5 bg-zinc-100 text-zinc-600 rounded-full text-[10px] font-black uppercase tracking-widest">
                    {product.condition}
                  </span>
                  {product.listingType && (
                    <span
                ...
                {(product.status === 'reserved' || product.status === 'pending') && (
                  <div className="p-4 bg-amber-500/10 border-2 border-amber-500/30 rounded-2xl flex items-center gap-3 text-amber-950 font-black shadow-xs">
                    <Clock size={20} className="text-amber-600 shrink-0" />
                    <div>
                      <span className="text-[10px] font-black uppercase tracking-widest text-amber-800 block">Statut de l'article</span>
                      <span className="text-sm font-bold">{formatReservedCountdown(product.reserved_until || (product as any).reservedUntil)}</span>
                    </div>
                  </div>
                )}
                <h2 className="text-4xl md:text-5xl font-black uppercase tracking-tighter leading-tight text-zinc-900 border-l-8 border-orange-600 pl-8">
                  {product.title}
                </h2>
                <div className="flex flex-col gap-4">
                  {product.listingType === "sale" && (
                    <div className="flex items-center gap-4">
                      <div className="text-4xl font-black text-orange-600 italic px-8 py-3 bg-orange-50 rounded-[2rem] w-fit shadow-sm">
                  ...
                  {product.listingType === "mixed" && (
                    <div className="flex flex-col gap-3 p-6 bg-orange-50/50 rounded-[2.5rem] border border-orange-100/50">
                      ...
                      <div className="text-4xl font-black text-orange-600 italic">
                        {formatPrice(product.price)} FCFA
                      </div>
                    </div>
                  )}
                  {product.listingType !== "sale" && (
                    <div className="flex flex-col gap-2 p-6 bg-blue-50 rounded-[2.5rem] border border-blue-100">
                      ...
                      <p className="text-2xl font-black uppercase tracking-tight text-blue-700">
                        {product.exchangeWanted ||
                          "Proposez ce que vous avez !"}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-6 bg-zinc-50 rounded-[2.5rem] border border-zinc-100 space-y-2">
                  <span className="text-[9px] font-black uppercase text-zinc-400 tracking-widest flex items-center gap-2">
                    <MapPin size={12} className="text-orange-600" /> Lieu de
                    rencontre
                  </span>
                  <p className="font-black uppercase tracking-tight text-sm">
                    {product.meetingPoint || "À convenir"}
                  </p>
                  <div className="flex items-center gap-1.5 mt-2">
                    <div
                      className={cn(
                        "w-1.5 h-1.5 rounded-full",
                        product.canTravel ? "bg-green-500" : "bg-red-400",
                      )}
                    />
                ...
                <div className="p-6 bg-zinc-50 rounded-[2.5rem] border border-zinc-100 space-y-2">
                  ...
                  <p className="font-black uppercase tracking-tight text-sm">
                    {product.availability || "À convenir"}
                  </p>
                </div>
              </div>
              ...
              <div className="p-5 rounded-[2rem] border bg-red-50/50 border-red-150/50 transition-all flex items-start gap-3.5 shadow-sm">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-sm self-start mt-0.5 bg-red-600 text-white shadow-red-650/15">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-xs font-black uppercase tracking-tighter leading-tight text-red-950 select-none">
                    ⚠️ Sécurité de transaction
                  </h4>
                  <p className="text-[11px] text-red-850 font-extrabold leading-relaxed mt-1">
                    Veuillez ne pas faire de paiement ou de dépôt à l'avance à moins de connaître personnellement le vendeur. Dès lors, TrocShop se désengage de tout problème éventuel ou litige financier.
                  </p>
                </div>
              </div>

              <div className="space-y-4">
                <h3 className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-2">
                  Description
                </h3>
              ...
              <div className="p-8 bg-zinc-900 rounded-[2.5rem] text-white flex flex-col gap-6 shadow-xl">
                <div
                ...
                >
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {isSellerPartner && (
                          <Shield
                            size={14}
                            className="text-emerald-500 fill-emerald-500 shrink-0 animate-pulse"
                          />
                        )}
                      </div>
                    </div>
                  ...
                  <Button
                    ...
                    variant="ghost"
                    className={cn(
                      "border rounded-full px-6 text-[10px] uppercase font-black tracking-widest transition-all",
                      isAlreadyInProfile
                        ? "border-zinc-800 text-zinc-500 bg-zinc-850/50 cursor-not-allowed opacity-50"
                        : "border-white/20 text-white hover:bg-white/10 cursor-pointer"
                    )}
                  >
                    {isAlreadyInProfile ? "Déjà sur le profil" : "Voir le profil"}
                  </Button>
                </div>
                    ...
                    <button
                      onClick={(e) => {
                        ...
                        const msg = encodeURIComponent(
                          `Bonjour, je souhaite signaler le vendeur "${product.sellerName}" de l'annonce "${product.title}" (ID: ${product.id}).`,
                        );
                        window.open(
                          `https://wa.me/2250160232164?text=${msg}`,
                          "_blank",
                        );
                      }}
                ...
                {isOwner ? (
                  <div className="bg-zinc-900/90 backdrop-blur-3xl text-white p-5 rounded-[2rem] flex items-center justify-between shadow-2xl border border-white/10">
                    ...
                    <div className="flex gap-2">
                      <Button
                        onClick={() => {
                          safeConfirm(
                            "Êtes-vous sûr de vouloir marquer cet article comme vendu ?",
                            () => {
                  ...
                  <>
                    <div className="flex gap-2 w-full">
                      {!isOwner && (product.status === 'available' || !product.status || (product.status as string) === 'active') && (
                        <div className="shrink-0">
                          <Button
                            onClick={() => {
                              onReserveProduct?.(product.id);
                            }}
                            className="h-14 px-5 bg-amber-500 hover:bg-amber-600 text-zinc-950 font-black rounded-2xl shadow-xl shadow-amber-500/20 flex items-center justify-center gap-2 border border-amber-400 cursor-pointer"
                          >
                            <Bookmark size={18} className="fill-zinc-950" />
                      ...
                      {product.listingType === "sale" && (
                        ...
                        >
                          <Button
                            onClick={() => {
                              onClose();
                              onContact(product.images[currentImg]);
                            }}
                      ...
                      {sellerProfile?.phoneVisibility === "public" && sellerProfile?.phoneNumber && (
                        ...
                        >
                          <Button
                            ...
                            className="h-14 w-14 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl flex items-center justify-center border border-emerald-500 shadow-xl shadow-emerald-600/20"
                            title={`Appeler le vendeur : ${sellerProfile.phoneNumber}`}
                          >
                            <Phone size={18} className="fill-white/10 text-white" />
                          </Button>
                        </motion.div>
                      )}
                    </div>
                  ...
                  */}
                      ...
                      <button
                        onClick={async () => {
                          ...
                          const isCapacitor = !!(window as any).Capacitor;
                          const titleText = `Découvre l'article "${product.title}" sur TrocShop !`;
                          ...
                          // Clipboard fallback
                          try {
                            await navigator.clipboard.writeText(`${titleText} Regarde ici : ${productUrl}`);
                            safeAlert("Lien de l'article copié dans le presse-papier !");
                          } catch (err) {
                            console.error("Clipboard copy failed", err);
                          }
                        }}
                        className="flex-1 h-14 rounded-2xl bg-zinc-50 text-zinc-800 hover:bg-zinc-100 flex items-center justify-center gap-3 active:scale-95 transition-all border border-zinc-200/40 shadow-sm"
                        title="Partager l'article"
                      >
                        <Share size={18} className="text-zinc-600" />
                        <span className="text-[10px] font-black uppercase tracking-widest">
                          Partager l'article
                        </span>
                      </button>
                      ...
                      {!isOwner && (
                        <button
                          onClick={() => {
                            safeConfirm(
                              "Êtes-vous sûr de vouloir signaler ce vendeur ? Vous allez être redirigé vers l'assistance TrocShop sur WhatsApp.",
                              () => {
                                const msg = encodeURIComponent(
                                  `Bonjour, je souhaite signaler le vendeur "${product.sellerName}" de l'annonce "${product.title}" (ID: ${product.id}).`,
                                );

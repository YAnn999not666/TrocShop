...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'home' | 'search' | 'favorites' | 'sell' | 'messages' | 'profile' | 'auth' | 'notifications' | 'student' | 'troc' | 'category' | 'shoplight'>(() => {
    // Handle PWA shortcuts or URL parameters
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      const tab = params.get('tab');
      if (tab && ['home', 'search', 'favorites', 'sell', 'messages', 'profile', 'notifications', 'student', 'troc', 'category', 'shoplight'].includes(tab)) {
        return tab as any;
      }
  ...
  return (
    <div className="min-h-screen w-full bg-white antialiased">
      ...
      <header className={cn(
        ...
        (activeTab === 'shoplight' || activeTab === 'messages' || activeTab === 'student' || activeTab === 'troc' || activeTab === 'category' || selectedSellerId || selectedProduct || showInAppCamera) ? "hidden" : ""
      )}>
      ...
      <main className={cn(
        activeTab === 'shoplight'
          ...
          : (activeTab === 'messages' || activeTab === 'student' || activeTab === 'troc' || activeTab === 'category')
            ...
            >
              <div className="space-y-4">
                ...
                <div className="w-full flex items-center justify-center bg-white rounded-[2.5rem] overflow-hidden mb-6 relative border border-zinc-100 shadow-sm p-4">
                  <img 
                    src="/gradient-arrows-design_23-2150390298.jpg" 
                    alt="Espace de Troc" 
...
function SellScreen({ onComplete, user, initialProduct, showInAppCamera, setShowInAppCamera }: { onComplete: () => void; user: User | null; initialProduct?: Product | null; showInAppCamera: boolean; setShowInAppCamera: (active: boolean) => void }) {

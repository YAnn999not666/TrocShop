...
import { Product, UserProfile, Message, Conversation, Review, ServiceListing } from './types';
...
const ConversationRecipientName = ({ participantNames, participantIds, currentUser }: { participantNames: string[], participantIds: string[], currentUser: any }) => {
  ...
  const isCertified = (profile?.is_certified as any) === true || (profile?.is_certified as any) === 'vrai' || (profile?.isCertified as any) === true || (profile?.isCertified as any) === 'vrai' || (profile?.['is certified'] as any) === true || (profile?.['is certified'] as any) === 'vrai' || profile?.isPro === true || profile?.is_pro === true || (profile?.badges as string[])?.some(b => b?.toLowerCase().includes('certif'));
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      const isDismissed = localStorage.getItem('install_banner_dismissed') === 'true';
  ...
  const handleInstallClick = async () => {
    if (!deferredPrompt) {
      alert("Pour installer TrocShop, ouvrez le menu de votre navigateur (icône de partage ou les trois points) et sélectionnez 'Ajouter à l'écran d'accueil' ou 'Installer l'application'.");
      setShowInstallBanner(false);
  ...
  const [updateDownloadUrl, setUpdateDownloadUrl] = useState("https://troc-shop-kgfn.vercel.app/");
  ...
  const [trocFilterCondition, setTrocFilterCondition] = useState<string>('Tous');
  const [trocFilterPriceMax, setTrocFilterPriceMax] = useState<number | null>(null);
  const [trocFilterListingType, setTrocFilterListingType] = useState<string>('Tous');
  const [trocFilterNeighborhood, setTrocFilterNeighborhood] = useState<string>('Tous');
  const [trocSortBy, setTrocSortBy] = useState<'newest' | 'price-asc' | 'price-desc' | 'oldest'>('newest');
  ...
  const [activeTab, setActiveTab] = useState<'home' | 'search' | 'favorites' | 'sell' | 'messages' | 'profile' | 'auth' | 'notifications' | 'student' | 'troc' | 'category' | 'shoplight' | 'services'>(() => {
    // Handle PWA shortcuts or URL parameters
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      const tab = params.get('tab');
      if (tab && ['home', 'search', 'favorites', 'sell', 'messages', 'profile', 'notifications', 'student', 'troc', 'category', 'shoplight', 'services'].includes(tab)) {
        return tab as any;
      }
  ...
  const [listingsTab, setListingsTab] = useState<'products' | 'services'>('products');
  ...
  const loadExplorerPage = (pageIndex: number, lastDocOption: any) => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      console.warn("Error subscribing to explorer products:", err?.message || err);
      setExplorerLoading(false);
  ...
  const handleDeleteService = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    safeConfirm("Voulez-vous vraiment supprimer cette annonce de service ?", async () => {
  ...
  const [publicProfileTab, setPublicProfileTab] = useState<'boutique' | 'services'>('boutique');
  ...
  const handleEmailLogin = async (firstName: string, lastName: string, pass: string) => {
    try {
      ...
      try {
        ...
        // If user not found, try to register them automatically to make it foolproof
  ...
  const filteredTrocProducts = useMemo(() => {
    ...
    const filtered = allUniqueProducts.filter(p => {
      ...
      const isTrocProduct = p.listingType === 'troc' || p.listingType === 'mixed';
      ...
      if (trocFilterListingType !== 'Tous') {
        if (trocFilterListingType === 'Troc' && p.listingType !== 'troc') return false;
        if (trocFilterListingType === 'Troc +' && p.listingType !== 'mixed') return false;
      }
  ...
  const handleContactSellerFromSpotlight = async (sellerId: string, sellerNom: string, item: any) => {
    if (!user) {
      triggerAuthRequired("Pour d'abord discuter avec le vendeur, vous devez d'abord vous connecter ou créer un compte.");
      return;
    }

    if (sellerId === user.uid) {
      alert("C'est votre profil ! Vous ne pouvez pas vous contacter vous-même.");
      return;
    }

    // Always create a spotlight-specific conversation thread for this video
    const convId = [user.uid, sellerId].sort().join('_') + '_spotlight_' + (item?.id || 'default');
  ...
  const handleContactServiceSeller = async (sellerId: string, sellerName: string, service: any) => {
    if (!user) {
      triggerAuthRequired("Pour d'abord discuter avec le prestataire, vous devez d'abord vous connecter ou créer un compte.");
      return;
    }

    if (sellerId === user.uid) {
      alert("C'est votre annonce de service ! Vous ne pouvez pas vous contacter vous-même.");
      return;
    }

    const convId = [user.uid, sellerId].sort().join('_') + '_service_' + service.id;
    const convRef = doc(db, 'conversations', convId);

    const productTitle = service?.title ? `Service: ${service.title}` : `Service`;
    const productPrice = typeof service?.price === 'number' ? service.price : (parseFloat(service?.price) || 0);
  ...
  const handleProposeExchange = async (targetProduct: Product, myProductIds: string[], priceAdjustment: number = 0) => {
    ...
    try {
      ...
      const convId = [user.uid, targetProduct.sellerId, targetProduct.id].sort().join('_');
      const convRef = doc(db, 'conversations', convId);
      ...
      if (!snap.exists()) {
        batch.set(convRef, {
          ...
          participantNames: [user.displayName || 'Utilisateur', targetProduct.sellerName],
          productId: targetProduct.id,
          sellerId: targetProduct.sellerId,
          productTitle: targetProduct.title,
          productImage: targetProduct.images[0],
    ...
    <div className="min-h-screen w-full bg-white antialiased">
      ...
      <aside className="hidden min-[980px]:flex fixed left-0 top-0 w-[220px] h-screen bg-white border-r border-zinc-200/80 flex-col justify-between p-6 z-[110] select-none">
        <div className="space-y-8">
          {/* Logo TrocShop */}
          <div className="flex items-center gap-2 cursor-pointer group py-2" onClick={() => { setActiveTab('home'); setActiveConversationId(null); setSelectedSellerId(null); setPreviousProduct(null); }}>
          ...
          <nav className="flex flex-col gap-2">
            {[
              ...
              { id: 'search', label: 'Rechercher', icon: <Search size={20} /> },
              { id: 'shoplight', label: 'Shoplight', icon: <Tv size={20} /> },
              { id: 'messages', label: 'Messages', icon: <MessageCircle size={20} />, count: unreadMessages }
            ].map((item) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className={isActive ? "text-orange-600" : "text-zinc-400 group-hover:text-zinc-600"}>
                      {item.icon}
                    </span>
      ...
      <header className={cn(
        ...
        (activeTab === 'shoplight' || activeTab === 'messages' || activeTab === 'student' || activeTab === 'troc' || activeTab === 'category' || activeTab === 'services' || selectedSellerId || selectedProduct || showInAppCamera) ? "hidden" : ""
      )}>
        <div className="flex items-center justify-between w-full gap-4">
          ...
          <div className="hidden min-[980px]:block text-zinc-400 text-[10px] font-black uppercase tracking-widest select-none shrink-0 min-w-[120px]">
            {activeTab === 'home' ? 'Tableau de bord' : activeTab === 'search' ? 'Recherche' : activeTab === 'shoplight' ? 'Shoplight' : activeTab === 'messages' ? 'Messagerie' : activeTab === 'sell' ? 'Publier' : 'Mon Espace'}
          </div>
      ...
      <main className={cn(
        activeTab === 'shoplight'
          ...
          : (activeTab === 'messages' || activeTab === 'student' || activeTab === 'troc' || activeTab === 'category' || activeTab === 'services')
      ...
      {activeTab === 'home' && (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <p className="text-[8px] font-black uppercase tracking-[0.25em] text-indigo-500">Exclusif</p>
                      <h4 className="font-black text-sm text-indigo-950 uppercase tracking-wider">Espace Services</h4>
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                <header className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-4xl font-black tracking-tighter uppercase italic text-orange-600 font-sans">Espace de Troc</h2>
                      <div className="h-1.5 w-12 bg-orange-600 rounded-full mt-1" />
                    </div>
                  </div>
                </header>
                ...
                <div className="flex items-center gap-3 w-full">
                  ...
                  <Button 
                    ...
                    className={cn(
                      "rounded-2xl h-14 w-14 shrink-0 transition-all flex items-center justify-center p-0 border border-zinc-100 shadow-sm",
                      isTrocFiltersOpen ? "bg-orange-100 text-orange-600 border-orange-200" : "bg-white text-zinc-400 hover:text-zinc-600"
                    )}
                ...
                <AnimatePresence>
                  {isTrocFiltersOpen && (
                    ...
                    >
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <label className="text-[10px] font-black uppercase tracking-widest text-zinc-800">Valeur Max (Ajustement)</label>
                          <input 
                            type="number" 
                            placeholder="F CFA" 
                            value={trocFilterPriceMax !== null ? trocFilterPriceMax : ''}
                            className="w-full bg-white border border-zinc-200 p-3 rounded-xl text-xs font-bold outline-none focus:border-orange-500"
                            onChange={(e) => setTrocFilterPriceMax(e.target.value ? Number(e.target.value) : null)}
                          />
                        </div>
                          ...
                          <div className="flex flex-wrap gap-2">
                             {['Tous', "Comme neuf", "Très bon état", "Satisfaisant", "C'est gâté (pour pièces)"].map(c => (
                               <button 
                                 ...
                                 className={cn(
                                   "px-3 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-wider transition-all border",
                                   trocFilterCondition === c ? "bg-orange-600 text-white border-orange-600 shadow-sm" : "bg-white text-zinc-500 border-zinc-200"
                                 )}
                               >
                                 {c}
                               </button>
                        ...
                        <div className="space-y-2 col-span-full">
                          <label className="text-[10px] font-black uppercase tracking-widest text-zinc-800">Type de Troc</label>
                          <div className="flex flex-wrap gap-2">
                             {['Tous', 'Troc', 'Troc +'].map(t => (
                               <button 
                                 ...
                                 className={cn(
                                   "px-3 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-wider transition-all border",
                                   trocFilterListingType === t ? "bg-orange-600 border-orange-600 text-white shadow-sm" : "bg-white text-zinc-500 border-zinc-200"
                                 )}
                               >
                                 {t}
                               </button>
          ...
          {activeTab === 'sell' && (
            ...
            >
              <SellScreen 
                onComplete={() => { setActiveTab('home'); setEditingProduct(null); setEditingService(null); }} 
                user={user} 
                    ...
                    <div className="flex bg-zinc-100 p-1 rounded-[1.2rem] max-w-xs">
                      <button
                        ...
                        className={cn(
                          ...
                          listingsTab === 'products' ? "bg-white text-zinc-900 shadow-sm" : "text-zinc-500 hover:text-zinc-800"
                        )}
                      >
                        Produits ({products.filter(p => p.sellerId === user?.uid).length})
                      </button>
                      <button
                        onClick={() => setListingsTab('services')}
                        className={cn(
                          "flex-1 py-3 px-4 rounded-[1rem] text-[10px] font-black uppercase tracking-widest transition-all",
                          listingsTab === 'services' ? "bg-white text-zinc-900 shadow-sm" : "text-zinc-500 hover:text-zinc-800"
                        )}
                      >
                        Services ({allServices.filter(s => s.sellerId === user?.uid).length})
                      </button>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 min-[980px]:grid-cols-3 min-[1440px]:grid-cols-4 gap-4">
                    {profileView === 'listings' ? (
                      listingsTab === 'products' ? (
                        ...
                        allServices.filter(s => s.sellerId === user?.uid).map(s => (
                          <div key={s.id} className="relative group bg-white rounded-3xl border border-zinc-150 overflow-hidden shadow-sm flex flex-col h-full cursor-pointer" onClick={() => {
                            ...
                            (window as any).__openServiceDetailId = s.id;
                            window.dispatchEvent(new CustomEvent('open-service-detail', { detail: s.id }));
                          }}>
                            <div className="aspect-square w-full relative bg-zinc-50 flex items-center justify-center overflow-hidden">
                              {s.images && s.images[0] ? (
                                <img src={s.images[0]} alt={s.title} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                              ) : (
                                <div className="text-zinc-300 font-bold text-xs uppercase">Pas d'image</div>
                              )}
                              <div className="absolute top-3 right-3 bg-zinc-900/80 backdrop-blur-md px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest text-white">
                                {s.serviceType === 'house' ? '🏠 Maison' : '🚗 Voiture'}
                              </div>
                            </div>
                              ...
                              <div className="mt-3 pt-3 border-t border-zinc-100">
                                <div>
                                  ...
                                  <span className="text-[10px] text-zinc-400 font-medium">
                                    {" "}FCFA/{s.pricePeriod === 'month' ? 'mois' : 'jour'}
                                  </span>
                                </div>
                              </div>
                            </div>
                    ...
                    {((profileView === 'listings' 
                      ? (listingsTab === 'products' ? products.filter(p => p.sellerId === user?.uid) : allServices.filter(s => s.sellerId === user?.uid))
      ...
      {activeTab !== 'auth' && (
        <nav className={cn(
          ...
          (activeConversationId || isNavHidden || activeTab === 'notifications' || activeTab === 'shoplight') ? "translate-y-32 opacity-0 pointer-events-none" : "translate-y-0 opacity-100"
        )}>
          <div className="h-full bg-white/40 backdrop-blur-[40px] backdrop-saturate-[180%] border border-white/50 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] px-6 flex items-center justify-around relative overflow-visible">
            ...
            <div ref={el => { bottomNavItemRefs.current[3] = el }} className="relative z-20 flex-1 flex justify-center">
              <NavButton 
                ...
                onClick={() => { setActiveTab('shoplight'); setActiveConversationId(null); setSelectedSellerId(null); setPreviousProduct(null); }} 
                icon={<Tv size={24} />} 
              />
            </div>
            ...
            <div className="max-w-xl mx-auto px-6 py-12 pb-32">
                ...
                <div className="flex flex-col items-center text-center space-y-6 mb-12">
                  <div className="w-32 h-32 rounded-[2.5rem] bg-orange-100 flex items-center justify-center text-orange-600 text-4xl font-black italic shadow-2xl relative">
                    ...
                    {(() => {
                      const isCert = (publicProfile.is_certified as any) === true || (publicProfile.is_certified as any) === 'vrai' || (publicProfile.isCertified as any) === true || (publicProfile.isCertified as any) === 'vrai' || (publicProfile['is certified'] as any) === true || (publicProfile['is certified'] as any) === 'vrai' || publicProfile.isPro === true || publicProfile.is_pro === true || (publicProfile.badges as string[])?.some(b => b?.toLowerCase().includes('certif'));
               ...
               {(() => {
                 const isPublicProfilePro = publicProfile.isPro === true || publicProfile.is_pro === true || (publicProfile.isPro as any) === 'true' || (publicProfile.is_pro as any) === 'true';
                 const filteredUserServices = allServices.filter(s => s.sellerId === selectedSellerId);

                 return (
                   <>
                     {isPublicProfilePro && (
                        <div className="bg-zinc-100 p-1 rounded-[2rem] flex gap-1 mb-8 max-w-xs mx-auto">
                          <button
                            type="button"
                            ...
                            className={cn(
                              ...
                              publicProfileTab === 'boutique' ? "bg-white text-zinc-900 shadow-sm" : "text-zinc-500 hover:text-zinc-800"
                            )}
                          >
                            Boutique ({sellerProducts.length})
                          </button>
                          <button
                            type="button"
                            ...
                            className={cn(
                              "flex-1 py-3 px-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all",
                              publicProfileTab === 'services' ? "bg-white text-zinc-900 shadow-sm" : "text-zinc-500 hover:text-zinc-800"
                            )}
                          >
                            Services ({filteredUserServices.length})
                          </button>
                        </div>
                     )}

                     {(!isPublicProfilePro || publicProfileTab === 'boutique') ? (
                       <div className="space-y-8">
                       ...
                       <div className="space-y-8">
                          <div className="flex items-center justify-between border-b-4 border-indigo-600 w-fit pb-2">
                            <h3 className="text-xl font-black italic uppercase tracking-tighter">Services</h3>
                          </div>
                          <div className="grid grid-cols-2 min-[980px]:grid-cols-3 min-[1440px]:grid-cols-4 gap-4">
                            {filteredUserServices.length > 0 ? filteredUserServices.map(s => (
                              <div 
                                key={s.id} 
                              ...
                              >
                                <div className="aspect-square w-full relative bg-zinc-50 flex items-center justify-center overflow-hidden">
                                  {s.images && s.images[0] ? (
                                    <img src={s.images[0]} alt={s.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" referrerPolicy="no-referrer" />
                                  ) : (
                                    <div className="text-zinc-300 font-bold text-[10px] uppercase">Pas d'image</div>
                                  )}
                                  <div className="absolute top-3 right-3 bg-zinc-900/80 backdrop-blur-md px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest text-white">
                                    {s.serviceType === 'car' ? 'Auto/Moto' : 'Immo'}
                                  </div>
                                </div>
                                <div className="p-4 flex-1 flex flex-col justify-between space-y-2">
                                  <div>
                                    <div className="flex items-center gap-1.5 text-indigo-600 mb-1">
                                      <MapPin size={10} />
                                      <span className="text-[9px] font-black uppercase tracking-wider">{s.neighborhood}</span>
                                    </div>
                                    <h3 className="font-black text-xs text-zinc-900 leading-tight uppercase line-clamp-2">{s.title}</h3>
                                  </div>
                                  <div className="pt-2 border-t border-zinc-100 flex items-baseline gap-1">
                                    <span className="text-sm font-black italic text-zinc-900">{formatPrice(s.price)}</span>
                                    <span className="text-[9px] font-bold text-zinc-400">/{s.pricePeriod === 'day' ? 'jour' : 'mois'}</span>
                                  </div>
                                </div>
                              </div>
                            )) : (
                               <div className="col-span-full py-20 text-center opacity-20 font-black uppercase tracking-widest">Aucun service proposé</div>
                            )}
                          </div>
                       </div>
                     )}
...
function SellScreen({ onComplete, user, initialProduct, initialService, showInAppCamera, setShowInAppCamera }: { onComplete: () => void; user: User | null; initialProduct?: Product | null; initialService?: any | null; showInAppCamera: boolean; setShowInAppCamera: (active: boolean) => void }) {
  const [isProUser, setIsProUser] = useState(false);
  const [pubType, setPubType] = useState<'item' | 'service'>(initialService ? 'service' : 'item');
    ...
    const fetchProfile = async () => {
      if (user) {
        ...
        if (p) {
          ...
          const isPro = p.isPro === true || p.is_pro === true || p.isPro === 'true' || p.is_pro === 'true' || p.isPro === 1 || p.is_pro === 1;
          setIsProUser(isPro);
        }
      }
    };
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (imagePreviews.length + files.length > 6) {
      alert("La publication d'une annonce requiert au maximum 6 photos. Pas plus de 6 photos.");
      return;
    }
  ...
  const startCamera = async () => {
    if (imagePreviews.length >= 6) {
      alert("La publication d'une annonce requiert au maximum 6 photos. Pas plus.");
      return;
    }
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (imagePreviews.length < 4 || imagePreviews.length > 6) {
      alert("La publication d'une annonce requiert entre 4 et 6 photos. (Minimum: 4, Maximum: 6)");
      return;
    }
    ...
    try {
      ...
      setPublishStatus(`Optimisation et upload des ${imagePreviews.length} photos...`);
  ...
  if (pubType === 'service' && !initialProduct) {
    return (
      <div className="space-y-6 max-w-sm mx-auto">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-400">
                {initialService ? 'Modifiez les informations de votre service' : 'Postez votre service en 30 secondes'}
              </p>
            </div>
          </div>
        ...
        {isProUser ? (
          <SellServiceForm user={user} onComplete={onComplete} initialService={initialService} />
        ) : (
          <div className="bg-white border border-zinc-150 rounded-[2.5rem] p-8 text-center space-y-5 shadow-sm">
            ...
            <div className="space-y-2">
              <h3 className="font-black text-sm uppercase tracking-tight text-zinc-900">Accès Services Réservé</h3>
              <p className="text-xs text-zinc-500 leading-relaxed font-medium">
                La publication de services est une fonctionnalité exclusive réservée aux comptes professionnels certifiés sur TrocShop.
              </p>
            </div>
          ...
          <div className="bg-zinc-100 p-1 rounded-[2rem] flex gap-1 mt-4">
            <button
              ...
              className={cn(
                ...
                pubType === 'item' ? "bg-white text-orange-600 shadow-sm" : "text-zinc-400 hover:text-zinc-600"
              )}
            >
              Articles
            </button>
            <button
              ...
              className={cn(
                "flex-1 py-4 px-2 rounded-[1.8rem] text-[10px] font-black uppercase tracking-widest transition-all",
                pubType === 'service' ? "bg-indigo-600 text-white shadow-sm" : "text-zinc-400 hover:text-zinc-600"
              )}
            >
              Services
            </button>
      ...
      <div className="space-y-6">
        <div className="space-y-3">
          <label className="text-[10px] font-black uppercase text-zinc-800 tracking-[0.2em] ml-1 flex items-center justify-between">
            ...
            <span className="text-[8px] font-black text-rose-600">Entre 4 et 6 photos obligatoires</span>
          </label>
              ...
              <>
                ...
                <div className="flex gap-3">
                  ...
                  */}
                  ...
                  <button 
                    onClick={() => {
                      const appStoreUrl = "https://troc-shop-kgfn.vercel.app/";

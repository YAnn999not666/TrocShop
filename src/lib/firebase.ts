...
export function convertFirestoreTimestamp(value: any): any {
  if (value && typeof value === 'object' && 'seconds' in value && 'nanoseconds' in value) {
    const sec = Number(value.seconds);
    const nano = Number(value.nanoseconds || 0);
    if (!isNaN(sec)) {
...
function deepResolveServerTs(obj: any): any {
  ...
  const ts = convertFirestoreTimestamp(obj);
  if (ts !== obj) return ts;
...
export async function updateUserPassword(oldPassword: string, newPassword: string): Promise<void> {
  ...
  if (!email) throw new Error("Utilisateur non connecté ou email manquant.");
...
function addWatermarkToBase64(base64: string): Promise<string> {
  ...
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      try {
        ...
        // Calculate proportional watermark size
        const minDim = Math.min(canvas.width, canvas.height);
        const fontSize = Math.max(12, Math.min(100, Math.round(minDim * 0.035)));
        ...
        const rectY = canvas.height - margin - rectHeight;
        ...
        ctx.fillText(text, rectX + paddingX / 2, rectY + rectHeight / 2);
...
export async function uploadToSupabaseStorage(
  ...
  // Watermark any image uploaded to Supabase Storage (e.g. products, chat media, etc.)
  if (base64OrUrl.startsWith('data:image/')) {
    try {
      base64OrUrl = await addWatermarkToBase64(base64OrUrl);
    } catch (e) {
      console.error('Error in watermarking during upload:', e);
    }
  }
